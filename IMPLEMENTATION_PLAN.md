...
## Next-phase active items (RE-PLAN 2026-06-08 by designcritic, RE-PLAN task t_058b1c92)
...
1. **No operator-visible green local live-proof receipt exists.** Every `receipts/testnet-live-proof-<UTC>/` directory under `receipts/` is partial — most stop at `cluster=101` (precondition failed) or `doctor=0` then nothing else; the 2026-06-08 receipts are empty. The committed `crates/autotrain/tests/fixtures/testnet-live-proof-fixture/` is a byte-stable green reference the `trainer --verify-receipt` CLI re-verifies, but it is a *fixture*, not *the receipt the runbook produced against a real Postgres this week*. `steward/HINGES.md` rank #2 names this as the second mainnet-blocking hinge, after `SEAT-PERSIST-001` (now closed).
2. **The 11,495-line `IMPLEMENTATION_PLAN.md` carries 17 open `[ ] [P1]` ghost rows** (`STW-040, 041, 045 (x3), 046 (x3), 048, 056, 057, 060, 063, 064, 065, 066, 068`). The `scripts/plan-staleness-gate.sh` script catches `[ ] [P0]` ghosts but does not yet catch `[ ] [P1]` ghosts (the 6th-pass `STW-065` row explicitly defers that extension). A fresh `auto parallel` tick that scans the plan sees 17 claimable P1 rows and 0 of them are real work — every one is RESCOPED, SUPERSEDED, or already shipped. `steward/PROMOTIONS.md` ranks this as `PLAN-GHOST-RETIRE-001` (ready).
3. **The seat-aware paragraph at the end of this plan (slices 1-6 narrative) is stale.** Slice 4 ships the gate; the paragraph still reads as if slice 1 is the open question. The `crates/autotrain/tests/seat_collapse.rs` test comments still describe the bug as present ("assertion fails today" / "the test fails today" prose). A reader of the plan gets a wrong model of what shipped.
4. **The two `[!]` rows (`STW-001` planning surface, `STW-007` artifact retirement) are operator-decision blockers with no decision recorded.** The orchestrator can spawn `auto parallel` against a hand-authored queue (no gbrain needed), but a recorded decision (yes/no on `.gbrain-source` deletion, yes/no on `.auto/tui*/` retention) unblocks a downstream planning slice.
5. **No external consumer can replay a robopoker blueprint today.** Slice 6 of the seat-aware work names it explicitly: "FOLLOW-007 export to arena `bridge_v1` + sanity/bb100 validation BEFORE any live deploy." A v1/v2/v3 trained-config → myosu `bridge_v1` JSON exporter + a CI smoke that round-trips through `bridge_v1` is the next unblock for a third-party dashboard / arena site / training-pool integration.

The 5 rows below are the concrete, individually-shippable P0/P1 next-phase items. Each names the files it touches, states how it is verified, and has a narrow scope boundary so a worker can claim and finish it without re-discovering context. They are worker-ready, not prose. Owner for the RE-PLAN row itself: designcritic, 2026-06-08.

- [ ] **[P0] `STW-070` `TESTNET-LIVE-PROOF-RECEIPT` — produce one fresh green local `receipts/testnet-live-proof-<UTC>/` directory the `scripts/testnet-live-proof.sh` runbook emits against a real Postgres, with all 8 step exits `0` (doctor, cluster, reset, smoke, status, bench, compare, replay), the pinned `testnet live_proof complete: smoke=N status=N bench=N compare=N replay=BYTES` headline in `SUMMARY.txt`, and a `recipe.json` that re-verifies with `LiveProofReceipt::read_and_verify` (so the operator-visible receipt and the CI-visible receipt share one verifier, closing the residual "tool claims ≠ operator evidence" gap `steward/HINGES.md` rank #2 names).** Owner files: `scripts/testnet-live-proof.sh` (no code change expected — only run it), `scripts/testnet-live-proof.md` (note the evidence path), `crates/autotrain/src/doctor.rs` (only if a deterministic doctor bug surfaces during the run; the slice is *evidence first, code second*), the new receipt directory under `receipts/testnet-live-proof-<UTC>/` (the deliverable). Scope boundary: do not change the runbook's chain step order; do not weaken `LiveProofReceipt::read_and_verify`; do not bypass `trainer --doctor`; prefer `RBP_TESTNET_FAST=1` for the canonical first green proof (the operator can re-run with full settings later); if the runbook is the blocker, fix the *minimum* deterministic bug + add one focused regression test (`cargo test -p rbp-autotrain --test live_proof_receipt` or a new sub-test in `crates/autotrain/tests/live_proof.rs`); do NOT touch the dashboard / publish / index / index-remote / remote chain (they are downstream consumers of the receipt, not blockers). Acceptance criteria: a `find receipts/testnet-live-proof-<UTC>/` shows 8 step subdirs each with `stdout.txt` + `stderr.txt` + `exit.txt` where every `exit.txt` is `0`; `SUMMARY.txt` head is the pinned `testnet live_proof complete: ...` line; `recipe.json` parses with `LiveProofRecipe`; `trainer --verify-receipt receipts/testnet-live-proof-<UTC>/` exits 0 and prints `live_proof receipt verification passed: ...`. Verification commands: `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` (the receipt producer), `./target/debug/trainer --verify-receipt receipts/testnet-live-proof-<UTC>/` (the verifier), `cargo test -p rbp-autotrain --test live_proof_receipt` (the shared CI pin), `cargo test -p rbp-autotrain --test doctor` (the pre-flight gate), `bash scripts/plan-staleness-gate.sh` (must stay green). Hand-test: `cat receipts/testnet-live-proof-<UTC>/SUMMARY.txt` shows the pinned headline. Dependencies: working `DATABASE_URL` or `DB_URL` pointing at a reachable Postgres; `STW-067` (fast-mode, landed); `STW-069` (fail-before-train integrity gate, landed); `STW-019` (runbook, landed); `STW-023` (shared verifier + `recipe.json`, landed); `STW-028` (`--verify-receipt` CLI, landed). Estimated scope: S (operator-runnable evidence) to M (one focused deterministic fix if a runbook bug surfaces). Completion signal: a fresh `trainer --verify-receipt` exits 0 on a `receipts/testnet-live-proof-<UTC-THIS-WEEK>/` directory and the receipt path is recorded in `steward/HAZARDS.md` row #2 as the current operator proof. **`lens:` CEO (the receipt is the missing operator-visible proof of the testnet north star; without it "tool shipped" and "testnet live" are different claims) + Eng (no code change expected; the runbook + verifier + doctor + integrity gate are all already landed — this is the *one* run that proves they wire up end-to-end against a real Postgres) + Design (a stranger can `cat receipts/.../SUMMARY.txt` and see a real receipt, not a placeholder).**
