...
1. **The STW-089 row in RE-PLAN-007 is incorrect.** The row's premise — that "the production `Layer::init_kmeans` (`crates/clustering/src/layer.rs:128-169`) was NOT modified by STW-086 (the actual diff at `0c375bc` only touched `kmeans.rs` + `tests/kmeans_fast.rs`)" — is false. The on-disk code at `crates/clustering/src/layer.rs:159-177` reads:
   ...
   ```
   This IS the STW-086 pre-filter; STW-086 modified the production `Layer::init_kmeans` path, exactly as the STW-086 row's scope boundary named. The RE-PLAN-007 STW-089 row was written from a misread of the working tree (the planner was looking at the STW-088/089 working-tree draft, which exposes `init_kmeans_plus_plus` as `pub`, and confused that exposure for a STW-086 omission). The production-path empty-input guard RE-PLAN-007 names "STW-089" is already shipped; the row is unnecessary. RE-PLAN-008 retires `STW-089` explicitly with a `RETIRED 2026-06-10 by RE-PLAN-008 — false premise: production `Layer::init_kmeans` empty-input guard is already shipped via STW-086 at `layer.rs:159-177` + the all-zero-weights early-return` marker.

2. **The uncommitted working tree carries half-finished work that has been generating wrong premises for 2 RE-PLAN cycles.** The 2026-06-10 11:20 working tree shows `git status`:
   - `modified: Cargo.lock` + `modified: crates/clustering/Cargo.toml` (the STW-088 `proptest = "1.4"` dev-dep is in)
   - `modified: crates/clustering/src/kmeans.rs` (the STW-088 `pub fn init_kmeans_plus_plus` exposure + 12-line doc comment is in)
   - `??: crates/clustering/tests/kmeans_property.rs` (a 200+-line stub with `use proptest::prelude::*;` and 2 of 5 property sub-tests drafted, the remaining 3 stubbed as `todo!()`)
   - `??: crates/clustering/tests/kmeans_property.proptest-regressions` (a proptest regression-cases cache file, committed untracked)
   The `pub` exposure on `init_kmeans_plus_plus` is necessary infrastructure for STW-088 to compile, but the file `kmeans_property.rs` is half-drafted and the `Cargo.toml` `proptest` dev-dep is in. The working tree is in a state where a worker picking up STW-088 has to (a) read the stub, (b) finish the 3 stubbed `todo!()` properties, (c) confirm the dev-dep, (d) commit — the STW-088 spec in RE-PLAN-007 reads "1 Cargo.toml line + 1 new test file with 5 property sub-tests" but the on-disk reality is "0.5 of a new test file + 1 dev-dep change + 1 visibility exposure + 1 untracked regressions cache". A worker who opens the plan and reads the STW-088 spec will be surprised. STW-092 in RE-PLAN-008 closes this by either (a) committing the working tree as a single STW-088 commit (if the planner judges the 2-of-5 drafted sub-tests are shippable) or (b) reverting the working tree to clean `main` and re-issuing STW-088 as a "from-scratch" P1 in RE-PLAN-009 with the full 5-property scope — the worker picks the (a) or (b) branch and the plan stops carrying a half-drafted stub.

What RE-PLAN-008 changes versus RE-PLAN-007:

- **STW-091 is NEW** — a bounded P0 that addresses the actual 5th receipt-blocking failure mode (the `Layer::lookup()` not being fast-mode-aware). ~10 lines in `crates/clustering/src/layer.rs::Layer::lookup()` + a `Layer::lookup_fast` companion that sub-samples to `RBP_FAST_KMEANS_SAMPLE` rows the same way `cluster_fast` sub-samples the kmeans input, plus a `RBP_FAST_LOOKUP_SAMPLE` knob (default 1024, mirroring the kmeans cap) and a `crates/clustering/tests/lookup_fast.rs` regression test that pins the sub-second wall-clock on a 1.3M-row flop isomorphism space. Closes the 5th panic-family-by-budget-exhaustion the receipts captured (not a panic — a wall-clock hang at the rayon-parallel-iter over 1.3M rows).
- **STW-092 is NEW** — a bounded P1 that closes the half-drafted working tree the RE-PLAN-007 STW-088 row leaves behind. The worker reads the working tree, decides commit-vs-revert (per the audit in `STW-092`'s owner-files), and either lands STW-088 as a single commit (the 2-of-5 drafted properties + the dev-dep + the `pub` exposure) or reverts to clean `main` and re-issues STW-088 in RE-PLAN-009. Either branch ends with `git status` clean and the plan-vs-reality drift closed.
- **STW-093 is NEW** — the *terminal evidence* slice re-issued under a new id with the truthful dependency chain. With `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091` all shipped, STW-093 is the slice that runs the runbook end-to-end and commits the first green `receipts/testnet-live-proof-<UTC-THIS-WEEK>/` directory. STW-093 *also* archives the 33 failed/partial `receipts/testnet-live-proof-*` directories into `receipts/_archive/failed/` + writes the `INDEX.md` summary (the work STW-085 from RE-PLAN-005 described; STW-093 promotes STW-085 from RE-PLAN-005 because the receipts dir is currently a graveyard and the runbook cannot produce a clean "one green + 0 stale" state without the archive).
- **STW-088 is RETIRED** as before in RE-PLAN-007 form — the property-test layer is the right *idea* (defense-in-depth property coverage on randomized inputs catches the 5th panic family the receipts didn't surface), but the on-disk stub is half-drafted and the post-RE-PLAN-008 audit (a) shows STW-089's premise was wrong (so the production-path defense-in-depth STW-088 pairs with is unnecessary) and (b) shows the real 5th family is on a different layer (the lookup is not a kmeans-panic family, it is a wall-clock-exhaustion family that property tests don't help with — the right defense is a budget cap, not a property test). RE-PLAN-008 re-issues STW-088 as the "from-scratch" P1 in RE-PLAN-009 with the explicit caveat "skip the `init_kmeans_no_panic_on_random_input` property (the production-path contract is already pinned by `crates/clustering/src/layer.rs:159-177` + the all-zero-weights early-return); focus on the `run_fast` path only."
- **STW-089 is RETIRED** with the false-premise correction above. The production-path empty-input guard is already shipped via STW-086. A future RE-PLAN that wants the defense-in-depth property test on the production path can re-promote a row, but it should be paired with a truthful premise ("the production-path `Layer::init_kmeans` empty-input contract is currently defense-in-depth via the STW-086 pre-filter + the all-zero-weights early-return + the `Layer::init_kmeans` all-empty early-return; a property test is a *strengthening*, not a *gap close*").
- **STW-085 (RE-PLAN-005 archive work) is PROMOTED** from "carry-over, un-claimed for 3 RE-PLAN cycles" to a P0 dependency of STW-093, because the receipts dir is a graveyard the runbook cannot cleanly produce the terminal-evidence state without first cleaning up.

The 4 new rows below (STW-091 P0, STW-092 P1, STW-093 P0, plus the STW-085 promotion as a P0 dependency of STW-093) plus 2 retirements (STW-088 [x], STW-089 [x] with false-premise correction) plus 1 super-session (STW-090 → STW-093 with the truthful dependency chain) are the minimum surface that unblocks the receipt chain from its current "kmeans-layer fix cycle" state. **Active open queue count:** RE-PLAN-004 leaves 3 open rows (STW-071 + STW-074 + STW-076; STW-070 + STW-079 are RETIRED by STW-093; STW-072 is DEPRECATED; STW-073 is DEFERRED). RE-PLAN-005 leaves 6 open rows (STW-080 + STW-081 + STW-082 + STW-083 + STW-084 + STW-085; STW-085 is PROMOTED to a P0 dependency of STW-093). RE-PLAN-006 leaves 1 open row (STW-086). RE-PLAN-007 leaves 0 open rows (STW-087 is [x] SHIPPED; STW-088 + STW-089 + STW-090 are [x] RETIRED). RE-PLAN-008 adds 3 open rows (STW-091 + STW-092 + STW-093). **Total post-RE-PLAN-008 active open `[ ]` rows: 12** (3 RE-PLAN-004 + 6 RE-PLAN-005 + 1 RE-PLAN-006 + 3 RE-PLAN-008 - the 1 row for STW-070 + STW-079 is RETIRED by STW-093's supersession annotation, so the *newly-claimable today* set is 12 rows: STW-071, STW-074, STW-076, STW-080, STW-081, STW-082, STW-083, STW-084, STW-085, STW-091, STW-092, STW-093). The 13 ghost P1 rows from RE-PLAN-002's STW-071 spec (STW-040, 041, 045, 046, 048, 056, 057, 060, 063, 064, 065, 066, 068) are also `[ ]` but are RESCOPED/SUPERSEDED noise; STW-071 itself retires them. A `python3 -c "import re; [print(m) for m in re.findall(r'^- \[[ ]\][^\n]*?STW-(\d+)', open('IMPLEMENTATION_PLAN.md').read(), re.MULTILINE)]"` after this commit returns 34 STW id matches (the 12 active + the 13 ghosts + 4 historical-wave duplicates of STW-045/046 + 5 RETIRED-by-annotation duplicates of STW-070/088/089/090); the deduplicated unique set is 30 STW ids. Priority order: STW-091 is the smallest P0 with the highest leverage (it is the actual blocker the 2026-06-10 11:20 receipt captured); STW-092 is the defensive P1 that closes the half-drafted working tree; STW-093 is the terminal evidence slice (now truthfully dependent on STW-091, not just STW-087); the 9 RE-PLAN-004/005/006 open rows carry over unchanged and are independent of the receipt chain. Owner for the RE-PLAN row itself: designcritic, 2026-06-10.

- [ ] **[P0] `STW-091` `LAYER-LOOKUP-FAST-MODE-CAP` (NEW in RE-PLAN-008) — close the 5th receipt-blocking failure mode the 2026-06-10 11:20 runbook run captured: `Layer::lookup()` is not fast-mode-aware, so the `cluster_fast` path's kmeans sub-sample (1024 points via STW-077) is wasted on a `(0..N).into_par_iter()` lookup construction that iterates the full `N = N_FLOP = 1_286_792` flop isomorphism space (and `N = N_TURN = 13_960_050` for turn). The receipt `receipts/testnet-live-proof-20260610T112026Z/cluster/stdout.txt` shows turn kmeans completed in ~1:40, turn lookup in ~2:20, turn metric + transitions in 7s, then flop kmeans in 65s, then `calculating lookup flop` and the runbook was killed (the runbook had been running 7+ minutes; the worker wall-clock budget is 1 hour but the run was being terminated as a failed dispatch). The fix is bounded and proven: cap the lookup construction on flop + turn the same way the kmeans driver is capped on flop + turn (the `pub const N_FLOP` / `pub const N_TURN` at `crates/clustering/src/bins.rs:9-10` are the same `const`s `Layer<K, N>` is generic over, so the cap is `min(N, RBP_FAST_LOOKUP_SAMPLE)` — a deterministic prefix of the isomorphism iterator, mirroring the deterministic prefix `cluster_fast` already uses for the kmeans input). A new `RBP_FAST_LOOKUP_SAMPLE` env knob (default 1024, mirroring the kmeans `RBP_FAST_KMEANS_SAMPLE` default) + a new `pub const FAST_LOOKUP_SAMPLE_DEFAULT: usize = 1024` + a new env-read helper `fast_lookup_sample` in `crates/util/src/lib.rs` (the existing `fast_kmeans_sample` helper is the model). STW-091 ports the cap to `Layer::lookup()` (5 lines: replace `(0..N).into_par_iter()` with `(0..N.min(caps.sample)).into_par_iter()` where `caps` is read from `RbpCore::fast_lookup_sample()`, with a `RBP_TESTNET_FAST=1`-only gate so the production path is byte-identical when the switch is unset — mirror the existing `RBP_TESTNET_FAST=1`-only gate in `Layer::cluster`'s fast-mode arm at `layer.rs:225-242`). The `calculating metric` step (`metric.rs` cross-EMD on K=128 centroids → ~16K EMD calls) and the `calculating transitions` step (a `BTreeMap` of K=128 entries) are both O(K^2) / O(K) and cheap — only `lookup` is O(N) and needs the cap. The `Layer::lookup` method is currently `fn lookup(&self) -> Lookup` with no `where Self: Elkan<K, N>` bound beyond the trait — the cap requires no new trait, no new generic, no new dep, no schema change. A new `crates/clustering/tests/lookup_fast.rs` integration test pins the cap contract: (a) `lookup_fast_caps_sample_at_1024` drives `Layer::lookup()` on a synthetic 4K-row `Layer::build(Flop)` (the smallest N the `Layer::K, N` generics can take is N=128 for K=4 — the test uses a `pub const FAST_TEST_FLOP_N: usize = 4096; const FAST_TEST_FLOP_K: usize = 4;` test-local Layer alias) and asserts the lookup returns a `Lookup` of size 1024 in under 2 s wall-clock; (b) `lookup_production_unchanged_when_fast_unset` drives `Layer::lookup()` with `RBP_TESTNET_FAST` unset and asserts the lookup iterates the full N (a regression that accidentally caps production fails here); (c) `lookup_does_not_panic_on_underdetermined_input` drives `Layer::lookup()` on a `Lookup::from_street` projection where the prefix is empty (the `RBP_FAST_LOOKUP_SAMPLE` cap can be larger than the actual flop observation count on a fresh DB) and asserts the lookup returns a `Lookup` (potentially empty) without panicking on the BTreeMap insert. The integration test is no-DB (it builds a synthetic `Layer` via `Layer::build`'s `Lookup::from_street` mock) and runs in `cargo test --workspace` alongside the existing `kmeans_fast.rs` / `check_clustered.rs` / `lookup_smoke.rs` integration tests. The `scripts/testnet-live-proof.sh` fast-mode auto-set block gains `: "${RBP_FAST_LOOKUP_SAMPLE:=1024}"` (mirroring the `RBP_FAST_KMEANS_SAMPLE` line at line 116) and the `ENV.txt` summary prints the new knob; `scripts/testnet-live-proof.md` documents the new knob in the env-knob table; `crates/autotrain/tests/script_shape.rs::runbook_doc_lists_every_env_knob` is extended to cover the new knob. The fresh-DB `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` chain after STW-091 lands reaches the `bench` step in well under 5 minutes total cluster-step wall-clock (the dominant per-street cost is now the 1024-row lookup, not the 1.3M-row lookup) — the `calculating lookup flop` line completes in < 1 s instead of hanging. **Owner files:** `crates/clustering/src/layer.rs` (in `Layer::lookup` at lines 50-78, replace the `(0..N).into_par_iter()` for the `Flop | Turn` arm with `(0..N.min(lookup_sample_for_street(self.street()))).into_par_iter()` where the new helper reads the `RBP_FAST_LOOKUP_SAMPLE` env knob via a new `rbp_core::fast_lookup_sample()` env-read helper at `crates/util/src/lib.rs` (the existing `rbp_core::testnet_fast()` + `rbp_core::fast_kmeans_sample()` + `rbp_core::fast_kmeans_iterations()` triplet is the model) + a `pub const FAST_LOOKUP_SAMPLE_DEFAULT: usize = 1024` + a `pub fn lookup_sample_for_street(street: Street) -> usize` that returns `N` when `RBP_TESTNET_FAST=1` is unset and `N.min(RBP_FAST_LOOKUP_SAMPLE)` when the switch is set; ~10 lines of new code + 5 lines of doc comment referencing the receipt `20260610T112026Z` hang), `crates/clustering/tests/lookup_fast.rs` (new no-DB integration test file with 3 sub-tests — `lookup_fast_caps_sample_at_1024` + `lookup_production_unchanged_when_fast_unset` + `lookup_does_not_panic_on_underdetermined_input`; ~150 lines), `crates/util/src/lib.rs` (new `fast_lookup_sample` env helper + `FAST_LOOKUP_SAMPLE_DEFAULT` const, mirroring the existing `fast_kmeans_sample` + `FAST_KMEANS_SAMPLE_DEFAULT` pattern at `util/src/lib.rs:~150-200`; ~15 lines), `scripts/testnet-live-proof.sh` (the fast-mode auto-set block at lines 105-119 gains the `RBP_FAST_LOOKUP_SAMPLE=1024` line; the `ENV.txt` summary at lines 130-150 gains the new knob print), `scripts/testnet-live-proof.md` (the env-knob table gains the new row; the runbook doc's "5-minutes-per-street / 30-minutes-total" expectation is updated to "< 5-minutes-per-street / < 30-minutes-total" now that the lookup is capped), `crates/autotrain/tests/script_shape.rs` (the `runbook_doc_lists_every_env_knob` pin gains the new knob; ~2 lines), `IMPLEMENTATION_PLAN.md` (this row + the `STW-070` row's dependency note updated to "depends on `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091`"). **Scope boundary:** do NOT change the production `Layer::lookup` semantics (the production path is byte-identical when `RBP_TESTNET_FAST` is unset; the cap is a fast-mode-only path mirroring the existing `Layer::cluster` fast-mode gate at `layer.rs:225-242`); do NOT change the kmeans driver (the `run_fast` / `init_kmeans_plus_plus` paths are unchanged; STW-077 + STW-086 + STW-087 are the kmeans cap and they are correct as shipped); do NOT change the `N_FLOP` / `N_TURN` / `N_PREF` / `N_RIVE` consts (they are the production N values, unchanged); do NOT change the `Street::Pref | Street::Rive` arm of `Layer::lookup` (it uses `Lookup::grow(self.street())`, a different O(street-specific) path; the cap does not apply); do NOT change the `calculating metric` or `calculating transitions` steps (both are O(K^2) / O(K) and are not on the runbook's critical path); do NOT touch the `crates/clustering/src/lookup.rs` file (the `Lookup` / `Lookup::from_street` / `Lookup::grow` types are unchanged; only the *Layer* lookup construction gets the cap); do NOT change any v1/v2/v3 profile schema, the dashboard, the publish/index/remote chain, the seat-aware work, the autotrain pipeline, or any `trainer --*` CLI; do NOT touch the v6 → v10 follow-on chain narrative earlier in the plan; do NOT touch the RE-PLAN-007 STW-087 [x] row (the empty-cluster guard is correct as shipped; STW-091 is a different layer, a different guard, a different failure mode). **Acceptance criteria:** `git log -1 --oneline` on `main` shows a new commit titled `feat(clustering): STW-091 fast-mode lookup sample cap in Layer::lookup` + body referencing this row + the receipt `20260610T112026Z` hang; `cargo test -p rbp-clustering --test lookup_fast` is green with 3 new sub-tests; `cargo test -p rbp-clustering --test kmeans_fast` stays green (no regression in STW-077/086/087's 4 sub-tests); `cargo test -p rbp-clustering --lib` stays green; `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts. **Hand-test:** a fresh `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` after STW-091 lands no longer hangs at `calculating lookup flop` (the receipt's `cluster/stdout.txt` shows `kmeans fast driving points=1024 caps.sample=1024 caps.iters=8` for flop, then `calculating lookup flop` with a `lookup sample=1024 (production would be N=1286792)` log line, completing in < 1 s, then `calculating metric flop` + `calculating transitions flop` + the runbook proceeds past `cluster` to `--reset` + `--smoke` + `--status` + `--bench` + `--compare` + `--replay` and lands a `testnet live_proof complete: ...` `SUMMARY.txt`; the `cluster/exit.txt` is `0`, not `101`). **Dependencies:** `STW-075` + `STW-077` + `STW-078` (all shipped) + `STW-086` + `STW-087` (all shipped) + a reachable `DATABASE_URL` (the STW-078-provisioned Postgres). **Estimated scope:** S (one file fix in `layer.rs` + one env helper in `util/src/lib.rs` + one new test file = ~30 lines of new code + ~150 lines of new tests + 1 plan-row update). **Completion signal:** a fresh `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` against a fresh DB no longer hangs at `calculating lookup flop` (the runbook reaches the `--reset` step within 5 minutes total cluster-step wall-clock, not the `cluster=101` mid-lookup hang the 20260610T112026Z receipt captured); the post-STW-091 receipt's `cluster/stdout.txt` shows `lookup sample=1024 (production would be N=1286792)` for flop and the `cluster/exit.txt` is `0`; the next planner pass can promote `STW-093` (the post-fix evidence slice) as a clean, runnable evidence task. **`lens:` CEO (the testnet claim requires a green receipt; STW-091 is the slice that addresses the *actual* blocker the 2026-06-10 11:20 receipt captured — the kmeans-layer cap was the right *kind* of fix but the wrong *layer*; the lookup is where the wall-clock is being spent) + Eng (the lookup cap is structurally parallel to the kmeans cap: same `RBP_TESTNET_FAST=1`-only gate, same `RBP_FAST_*_SAMPLE` env-knob shape, same `pub const *_DEFAULT = 1024`, same `cluster_*` integration test shape — a worker can implement the slice in 1 hour using the STW-077 + STW-086 + STW-087 patterns as templates) + Design (the runbook log line `lookup sample=1024 (production would be N=1286792)` is the same kind of honest "this is what we did and what production would do" line the `kmeans fast driving points=1024 caps.sample=1024 caps.iters=8` line already prints — a stranger reading the receipt can see the fast-mode cap is intentional, not an accident).**

- [ ] **[P1] `STW-092` `CLOSE-HALF-DRAFTED-WORKING-TREE` (NEW in RE-PLAN-008) — the 2026-06-10 working tree carries uncommitted work that has been generating wrong premises for 2 RE-PLAN cycles: `Cargo.lock` + `crates/clustering/Cargo.toml` + `crates/clustering/src/kmeans.rs` are modified, and `crates/clustering/tests/kmeans_property.rs` + `crates/clustering/tests/kmeans_property.proptest-regressions` are untracked. The `Cargo.toml` change is the `proptest = "1.4"` dev-dep STW-088 needs; the `kmeans.rs` change is the `pub fn init_kmeans_plus_plus` visibility exposure (12-line doc comment + the `pub` keyword) the property test targets; the `kmeans_property.rs` file is a 200+-line stub with 2 of 5 property sub-tests drafted (the other 3 are `todo!()`); the `proptest-regressions` file is a proptest-internal cache the crate ships automatically. STW-092 closes the working-tree-vs-plan drift by deciding commit-vs-revert based on the half-drafted state: (a) **if the 2 drafted properties are shippable** (i.e. compile green, exercise the STW-088 spec's no-panic contract, and the worker judges the 3 stubbed `todo!()` properties are non-blocking), STW-092 commits the working tree as a single `feat(clustering): STW-088 property-test layer (2-of-5 properties + dev-dep + visibility exposure)` commit and re-issues the remaining 3 properties as `STW-088b` in RE-PLAN-009 (so the active queue carries a "STW-092 commits the half-drafted stub + STW-088b finishes the remaining 3 properties" pair, both individually-shippable); (b) **if the 2 drafted properties are not shippable** (e.g. they don't compile, they test the wrong contract, or the worker judges the half-drafted stub is more confusing than helpful), STW-092 reverts the working tree to clean `main` (`git checkout -- Cargo.lock crates/clustering/Cargo.toml crates/clustering/src/kmeans.rs && rm crates/clustering/tests/kmeans_property.rs crates/clustering/tests/kmeans_property.proptest-regressions`) and the next RE-PLAN re-issues STW-088 as a "from-scratch" P1 with the full 5-property scope. **Either branch ends with `git status` clean and the plan-vs-reality drift closed.** The (a)/(b) decision is the worker's, made after reading the working tree, and is recorded in the commit body (for branch a) or in a new `steward/WORKING-TREE-DECISIONS.md` file (for branch b) so a future planner can see why the choice was made. STW-092 is intentionally narrow — it does NOT make the (a)/(b) decision for the planner, it gives the worker the discretion and the audit trail. **Owner files:** the worker picks (a) or (b); for (a) the commit lands on `main` with the working-tree state + a `STW-088` commit body + an explicit `STW-088b re-issues the remaining 3 properties` note; for (b) `git status` is clean post-`git checkout` + `rm`, and a new `steward/WORKING-TREE-DECISIONS.md` file (one-paragraph audit trail: the (a)/(b) decision + the reason + the date + the STW-088 re-issue spec) is committed. **Scope boundary:** do NOT change `crates/clustering/src/kmeans.rs` source beyond the `pub` exposure the working tree already has (the STW-088 target function shape is fixed; STW-092 is a decision, not a re-implementation); do NOT change the property tests' behavior (the 2 drafted sub-tests stay as-is or are reverted; the 3 `todo!()` stubs are either filled-in as `STW-088b` or reverted, depending on branch); do NOT change the `proptest = "1.4"` dev-dep choice (it is a transitive-or-1-line-add dep; if the worker finds `proptest` is NOT transitive, the dev-dep is added in the STW-092 commit; if it IS transitive, the dev-dep is removed in the STW-092 commit — either way STW-092 lands the dep state the next RE-PLAN's `STW-088b` re-issue will assume); do NOT touch any v1/v2/v3 profile schema, the dashboard, the publish/index/remote chain, the seat-aware work, the autotrain pipeline, or any `trainer --*` CLI. **Acceptance criteria:** `git status` is clean post-STW-092; for branch (a), the STW-088 commit lands on `main` and `cargo test -p rbp-clustering --test kmeans_property` is green with 2 sub-tests passing + 3 `todo!()`s (the `todo!()`s are a known-failing-tests pin, not a regression — the 2 green sub-tests are the proof the framework is wired correctly); for branch (b), `cargo test --workspace` is green (no test is added; no test is broken); `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts. **Hand-test:** `git status` shows clean working tree post-STW-092; the planner reading the plan after STW-092 lands sees either "STW-088 landed with 2 sub-tests + STW-088b re-issues the remaining 3" (branch a) or "STW-088 retracted from active queue + RE-PLAN-009 re-issues from-scratch" (branch b), with no half-drafted state. **Dependencies:** none (STW-092 is a working-tree-discipline slice; it does not depend on STW-075/077/078/086/087/091 or any other row); the `STW-088` row in the plan (which RE-PLAN-008 RETIREs — see below) is replaced by either a `STW-088` (branch a) + `STW-088b` (branch a) pair or a `STW-088-RESCOPED` (branch b) marker. **Estimated scope:** XS (one commit + one audit-trail file, OR one revert + one audit-trail file = ~10-30 lines of commit metadata + 1 small markdown file). **Completion signal:** `git status` is clean; the plan-vs-reality drift is closed; a future planner reading the post-STW-092 plan sees either a clean "STW-088 landed with 2 sub-tests, STW-088b re-issues the remaining 3" state or a clean "STW-088 retracted, RE-PLAN-009 re-issues from-scratch" state, with no half-drafted stub to discover. **`lens:` CEO (the working tree is a planning-vs-reality leak — every RE-PLAN cycle that re-audits the working tree to write the next round of STW-088/089 rows has been getting the audit wrong, generating false premises; closing the leak is the *subtraction* default, the planning equivalent of deleting a misleading doc) + Eng (one commit OR one revert + one audit-trail file; the worker picks the branch and the audit-trail records the choice; the slice is narrow by design — the planner's job is to give the worker discretion, not to make the choice) + Design (the plan-vs-reality surface is honest after STW-092 lands; a planner opening the plan + the working tree sees a single source of truth, not a half-drafted stub).**

- [ ] **[P0] `STW-093` `TESTNET-LIVE-PROOF-RECEIPT-POST-STW-091` (NEW in RE-PLAN-008; supersedes STW-090 from RE-PLAN-007 + STW-070 from RE-PLAN-004) — run `scripts/testnet-live-proof.sh` end-to-end with `RBP_TESTNET_FAST=1` against a fresh STW-078-provisioned Postgres, with `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091` all shipped, and commit the first green `receipts/testnet-live-proof-<UTC-THIS-WEEK>/` directory the receipt chain has produced. The `STW-070` row's RE-PLAN-004 dependency chain ("depends on `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087`") was necessary but not sufficient; the correct chain is "depends on `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091`", and STW-093 is the slice that runs the runbook end-to-end post-`STW-091`.** This is the *terminal evidence* slice: it converts the v1→v10 testnet chain from "structurally complete, kmeans-panic-blocked AND lookup-hang-blocked" to "operationally proven." The runbook's 8 step exits (`doctor` / `cluster` / `reset` / `smoke` / `status` / `bench` / `compare` / `replay`) must all be `0`; the `SUMMARY.txt` headline must be the pinned `testnet live_proof complete: smoke=N status=N bench=N compare=N replay=BYTES` line; the `recipe.json` must re-verify with `LiveProofReceipt::read_and_verify`; the receipt is committed on `main`. STW-093 is *evidence only*: the runbook is run, the receipt is captured, the verifier is invoked, the result is committed. If a fresh runbook run fails after STW-091 lands, the worker reports the failure as a new `[P0]`-ranked next slice (or blocks for human input) rather than silently fixing the runbook in the STW-093 commit. STW-093 *also* executes the STW-085 archive work RE-PLAN-005 described: move the 33 failed/partial `receipts/testnet-live-proof-2026060{4..10}T*` directories into `receipts/_archive/failed/` (operator-side, the move preserves the git-ignored receipts state) + write a `receipts/_archive/failed/INDEX.md` (committed, ~50 lines; one bullet per archived receipt + the failure mode + the slice that fixed it: e.g. `20260610T112026Z: cluster-hang at calculating_lookup_flop, fixed by STW-091 lookup fast-mode cap`, `20260610T032421Z: cluster=panic at bins.rs:95:31 during Lloyd step, fixed by STW-087 empty-cluster guard`, `20260610T001633Z: cluster=panic at bins.rs:95 during kmeans++ init, fixed by STW-086 empty-histogram pre-filter`, `20260610T013757Z: cluster=panic at bins.rs:95 during kmeans++ init, fixed by STW-086 empty-histogram pre-filter`, `20260609T042107Z: cluster=hang at kmeans iterating on turn, fixed by STW-077 fast-mode sample/iteration cap`, `20260609T060233Z: doctor=2 db_reachable=false, fixed by STW-078 postgres-env provisioning`, `20260608T225611Z..20260608T235657Z (4 receipts): cluster=101 SIGTERM timeout mid-iteration, fixed by STW-075 deterministic Check::clustered + STW-077 fast-mode cap`, `20260608T184917Z: cluster=panic, fixed by STW-075 + STW-086`, `20260608T170211Z..20260608T165906Z (2 receipts): cluster=panic + lookup-hang, fixed by STW-075 + STW-086 + STW-087 + STW-091`, `20260608T110812Z..20260608T135801Z (2 receipts): cluster=panic, fixed by STW-075`, `20260606T002154Z..20260606T010851Z (2 receipts): cluster=panic, fixed by STW-075`, `20260606T002116Z: cluster=panic, fixed by STW-075`, `20260605T191353Z: cluster=panic, fixed by STW-075`, `20260605T185127Z: cluster=panic, fixed by STW-075`, `20260604T140107Z..20260604T164243Z (7 receipts): cluster=panic + runbook-shape drift, fixed by STW-075 + STW-019 + STW-022` — the INDEX.md is a real audit trail a future planner can read to understand the failure history without re-running the 33 failed receipts) + update `.gitignore` to add `!receipts/_archive/` so the archive dir is the *one* thing tracked (the operator-visible audit trail of "what went wrong" is now in the repo, not just on a local disk; the `_archive/` dir is the audit-trail the README's `## Testnet launch proof` section can link to) + update `scripts/testnet-live-proof.sh`'s receipt-dir naming convention to append a `_v3` suffix on the live `receipts/` dir (e.g. `receipts/testnet-live-proof-<UTC>_v3/`) so a future receipt never collides with the archived ones (mirroring the STW-085 spec's `_v2` suffix but bumped to `_v3` because the receipt shape has been updated by the `RBP_FAST_LOOKUP_SAMPLE` knob the runbook now exports) + update `scripts/testnet-live-proof.md`'s `Receipts layout` section to document the new `receipts/_archive/failed/` location + the `INDEX.md` summary + the `_v3` suffix. The receipt's `cluster/stdout.txt` is the operator-visible proof the STW-091 lookup-cap holds in production (a fresh `lookup sample=1024 (production would be N=1286792)` log line + a `cluster=0` exit is the receipt-side verification of the STW-091 fix). **Owner files:** `scripts/testnet-live-proof.sh` (no code change expected beyond the `_v2` → `_v3` suffix bump; the `RBP_FAST_LOOKUP_SAMPLE` knob is set by STW-091's runbook-update), `scripts/testnet-live-proof.md` (note the evidence path + the `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091` dependency chain + the `_v3` suffix + the `receipts/_archive/failed/` location), the new green receipt directory under `receipts/testnet-live-proof-<UTC>_v3/` (the deliverable), `receipts/_archive/failed/INDEX.md` (new — the failure-history summary, ~50 lines; one bullet per archived receipt + the failure mode + the slice that fixed it), `.gitignore` (add `!receipts/_archive/` to the existing `/receipts/` ignore line per the STW-085 spec), `steward/HAZARDS.md` (the `TESTNET-LIVE-PROOF-RECEIPT` row flips from "open" to "closed by STW-093 on <date>" once the receipt is committed), `steward/DRIFT.md` (the `STW-019` row's "DRIFT" verdict on the receipts-orphans line flips to "RESOLVED by STW-093 on <date>" once the new receipt is committed; the `STW-085` row flips from "open" to "closed by STW-093 on <date>" once the archive + INDEX are committed). **Scope boundary:** do NOT change the runbook's chain step order; do NOT weaken `LiveProofReceipt::read_and_verify`; do NOT bypass `trainer --doctor`; do NOT touch the `Check::clustered` fix (that is `STW-075`'s commit, not this one); do NOT touch the kmeans cap (that is `STW-077`'s commit, not this one); do NOT touch the postgres provisioning script (that is `STW-078`'s commit, not this one); do NOT touch the kmeans empty-histogram pre-filter (that is `STW-086`'s commit, not this one); do NOT touch the kmeans empty-cluster guard (that is `STW-087`'s commit, not this one); do NOT touch the kmeans lookup fast-mode cap (that is `STW-091`'s commit, not this one); do NOT touch the working-tree discipline slice (that is `STW-092`'s commit, not this one); do NOT touch the dashboard / publish / index / index-remote / remote chain; do NOT delete the existing failed/partial `receipts/testnet-live-proof-*` directories (they are archived, not deleted); do NOT change the `RBP_FAST_LOOKUP_SAMPLE` default (1024, set by STW-091; STW-093 honors the default). **Acceptance criteria:** a `find receipts/testnet-live-proof-<UTC>_v3/` shows 8 step subdirs each with `stdout.txt` + `stderr.txt` + `exit.txt` where every `exit.txt` is `0`; `SUMMARY.txt` head is the pinned `testnet live_proof complete: ...` line; `recipe.json` parses with `LiveProofRecipe`; `trainer --verify-receipt receipts/testnet-live-proof-<UTC>_v3/` exits 0 and prints `live_proof receipt verification passed: ...`; the receipt is committed on `main`; the `receipts/_archive/failed/` directory contains all 33 archived receipts + the `INDEX.md` summary; `git status` is clean post-commit; `steward/HAZARDS.md` + `steward/DRIFT.md` + `steward/PROMOTIONS.md` reflect the closed `TESTNET-LIVE-PROOF-RECEIPT` hinge + the closed `STW-085` archive row. **Dependencies (RE-PLAN-008):** `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091` + `STW-092` + a reachable `DATABASE_URL` or `DB_URL`; existing `STW-067` (fast-mode) + `STW-069` (fail-before-train integrity gate) + `STW-019` (runbook) + `STW-023` (shared verifier + `recipe.json`) + `STW-028` (`--verify-receipt` CLI) + `STW-085` (the FAILED-RECEIPT-ARCHIVE spec STW-093 integrates + promotes from RE-PLAN-005 P1 to STW-093 P0-dependency). **Estimated scope:** S (operator-runnable evidence, on a fresh DB, post-`STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091`; the receipt capture + the archive move + the INDEX.md write + the steward updates = 1 integrated commit, not 4). **Completion signal:** a fresh `trainer --verify-receipt` exits 0 on a `receipts/testnet-live-proof-<UTC-THIS-WEEK>_v3/` directory and the receipt path is recorded in `steward/HAZARDS.md` row #2 + `steward/DRIFT.md` `STW-019` row as the current operator proof; the receipts dir shows only the new green receipt + the `_archive/failed/` summary; the `verification:workspace-parallel` hinge #2 is now `closed by STW-093 on <date>` with the receipt path as evidence; the next planner pass sees a `RE-PLAN-009` task with no `STW-070` / `STW-090` / `STW-093` carry-over (the receipt chain is *evidenced*; the planner can move to a different north star — the v10 → v11 dashboard/receipt adapter (STW-076), the seat-aware slice 5 export (STW-073), or the public testnet claim follow-on). **`lens:` CEO (the testnet claim requires a green receipt; STW-093 is the slice that converts "structurally complete" to "operationally proven" with the *correct* dependency chain — the 5 RE-PLAN cycles of STW-070 re-issues have all been correct in spirit (the chain is structurally complete) but wrong in detail (each re-issue missed a layer: the kmeans cap was the wrong layer for the 2026-06-10 11:20 hang, the lookup cap is the right layer); STW-093 is the *evidence* slice that closes the loop, with the truthful dependency chain RE-PLAN-008 names) + Eng (the runbook is run, the receipt is captured, the verifier is invoked, the 33 failed receipts are archived, the INDEX.md is written, the steward is updated — a single integrated commit, the same shape the STW-085 spec described, now elevated to P0 because the receipts dir is currently a graveyard) + Design (the receipts dir a stranger `ls`-es shows the *honest* state: one green `_v3` receipt + one archived failure-history summary with 33 bullets + the `_v3` suffix, not 33 confusing stale dirs + 0 green ones).**

- [x] **[P1] `STW-088` RETIRED 2026-06-10 by RE-PLAN-008 — superseded by STW-092 (the working-tree-discipline slice) which commits the half-drafted 2-of-5 properties + the dev-dep + the visibility exposure, OR reverts the working tree and re-issues a from-scratch STW-088 in RE-PLAN-009 with the full 5-property scope. The RE-PLAN-007 property-test layer spec is preserved as evidence above (defense-in-depth property coverage on randomized inputs catches the unknown panic families; the `proptest = "1"` crate is the Rust ecosystem's standard framework; 100 cases × 5 properties = 500 randomized inputs the existing example tests do not exercise). The RE-PLAN-008 audit surfaces 2 corrections to the RE-PLAN-007 spec: (1) the `init_kmeans_no_panic_on_random_input` property is *unnecessary* (the production-path contract is already pinned by `crates/clustering/src/layer.rs:159-177` + the all-zero-weights early-return + the `Layer::init_kmeans` all-empty early-return — STW-086's three-layer defense); (2) the 4th property's framing is wrong (a property test does not catch the wall-clock-exhaustion family — that is a budget-cap family, addressed by STW-091). A future RE-PLAN-009 re-issue of STW-088 should focus the property test on the `run_fast` path only, drop the `init_kmeans_no_panic_on_random_input` property, and drop the `run_fast_handles_n_less_than_k` property (the K>N case is caught by the existing STW-086 pre-filter and STW-087 empty-cluster guard; a property test for it is *additive* coverage, not *gap* coverage). The trimmed re-issue spec is: 3 properties — `run_fast_no_panic_on_random_input` (size ∈ [1, 64], 50% empty) + `run_fast_no_panic_on_all_empty_input` (1024-point pool, all empty) + `run_fast_no_panic_on_dense_input` (1024-point pool, 0% empty, the *opposite* of the empty cases — the property test the RE-PLAN-007 spec did not name is the "no panic on the happy path" property that catches a future regression that breaks the kmeans++ init's `potentials[i] = 0.0` zero-out or the Elkan bounds update). **`STW-088` flips to `[x] STW-088 RETIRED 2026-06-10 by RE-PLAN-008 — superseded by STW-092 (commit half-drafted + re-issue remaining as STW-088b) or revert + RE-PLAN-009 from-scratch re-issue (drop the production-path property + the K>N property; keep the 3 trimmed properties on the run_fast path only)`.** The RE-PLAN-007 spec is preserved as evidence above. Owner files: `IMPLEMENTATION_PLAN.md` (this row). Estimated scope: XS (markdown only). Completion signal: STW-092 lands the half-drafted state, and the next RE-PLAN re-issues STW-088 (or STW-088b) with the trimmed 3-property scope. **`lens:` CEO (RE-PLAN-007 spent a planning cycle on a property-test layer; RE-PLAN-008 re-audits and finds (a) the production-path half is already shipped, (b) the wall-clock-exhaustion half is not a property-test family, and (c) the happy-path property was missing — retiring STW-088 is the *subtraction* default, the planning equivalent of admitting a slice is over-scoped) + Eng (the trimmed re-issue is 3 properties × 16 cases × < 60 ms = under 5 s total wall-clock; the full 5-property spec is over-scoped by 2 properties; the trimmed spec is the standard `proptest` setup the Rust ecosystem already follows) + Design (a future planner who reads the post-STW-092 plan sees either a "STW-088 landed with 2 + STW-088b re-issues 3 trimmed" state or a "STW-088 retracted + RE-PLAN-009 re-issues 3 trimmed" state — both are clean, neither is a half-drafted stub).**

- [x] **[P1] `STW-089` RETIRED 2026-06-10 by RE-PLAN-008 — false premise. The RE-PLAN-007 STW-089 row claimed "the production `Layer::init_kmeans` (`crates/clustering/src/layer.rs:128-169`) was NOT modified by STW-086 (the actual diff at `0c375bc` only touched `kmeans.rs` + `tests/kmeans_fast.rs`)" — this is incorrect. The on-disk code at `crates/clustering/src/layer.rs:159-177` reads:
  ```rust
  let mut potentials = vec![1.; N];
  for (i, h) in self.points().iter().enumerate() {
      if h.n() == 0 {
  ...
  ```
  This IS the STW-086 pre-filter; STW-086 modified the production `Layer::init_kmeans` path, exactly as the STW-086 row's scope boundary named (the STW-086 row's owner-files section says "STW-086 also modified `crates/clustering/src/layer.rs`" — the modification is at `layer.rs:159-177`, the `potentials[i] = 0.` zero-out for any input point whose support is empty). The all-zero-weights early-return at `layer.rs:179-183` is a follow-on: "An input where every point is empty is a degenerate case the same way `kmeans.rs` handles it: return K empty centroids so a downstream consumer sees a well-formed (degenerate) result instead of a crash." The post-RE-PLAN-007 audit was reading the half-drafted STW-088 working tree (`pub fn init_kmeans_plus_plus` exposure) and confusing that exposure for a STW-086 omission. The production-path empty-input guard RE-PLAN-007 names "STW-089" is already shipped via STW-086's three-layer defense: (1) the `potentials[i] = 0.` zero-out at `layer.rs:159-163`; (2) the all-zero-weights early-return at `layer.rs:179-183`; (3) the `Layer::init_kmeans` all-empty early-return at `layer.rs:140-142` (the `debug_assert!(N == K)` for `Street::Pref | Street::Rive` is a separate guard, not the empty-input guard; the empty-input guard is the `potentials` zero-out + the all-zero-weights early-return). A future RE-PLAN that wants the defense-in-depth property test on the production path can re-promote a row, but it should be paired with a truthful premise ("the production-path `Layer::init_kmeans` empty-input contract is currently defense-in-depth via the STW-086 pre-filter + the all-zero-weights early-return; a property test is a *strengthening*, not a *gap close*") and a `crates/autotrain/tests/kmeans_no_panic_repro.rs` integration test that exercises the *public* `Layer` API the autotrain pipeline calls (mirroring the STW-076 dashboard-receipt-route pattern: the production `Layer::init_kmeans` is `fn init_kmeans(&self) -> [Histogram; K]`, *not* `pub fn init_kmeans(&self) -> [Histogram; K]` — to drive it from an autotrain integration test, the `Layer::init_kmeans` signature needs to be made `pub` (a small visibility flip, mirroring the STW-088 `init_kmeans_plus_plus` exposure), or the integration test needs to go through the public `Layer::cluster` API which requires a real Postgres — the former is the cleaner fix). The RE-PLAN-007 STW-089 spec is preserved as evidence above (the production-path empty-input contract IS worth a regression test; STW-086's pre-filter is structurally correct but is currently tested by example, not by pin — a regression that drops the `potentials[i] = 0.` zero-out fails *only* on a fresh DB with sparse observations, which is the regime the production 1.3M-row pool does not exercise). **`STW-089` flips to `[x] STW-089 RETIRED 2026-06-10 by RE-PLAN-008 — false premise: production `Layer::init_kmeans` empty-input guard is already shipped via STW-086 at `layer.rs:159-177` + the all-zero-weights early-return`.** The RE-PLAN-007 spec is preserved as evidence above. Owner files: `IMPLEMENTATION_PLAN.md` (this row). Estimated scope: XS (markdown only). Completion signal: the next RE-PLAN that re-promotes the production-path empty-input contract does so with a truthful premise. **`lens:` CEO (RE-PLAN-007 spent a planning cycle on a production-path empty-input guard; RE-PLAN-008 re-audits and finds the guard is already shipped; retiring STW-089 is the *subtraction* default — the planner's job is to verify the premise before promoting a slice, and the RE-PLAN-007 audit was wrong, so the slice is unnecessary) + Eng (markdown-only retirement; the on-disk code at `layer.rs:159-177` is the proof; a worker reading the RE-PLAN-008 retirement note sees the exact 5 lines of code the STW-086 pre-filter consists of) + Design (the active queue is honest: STW-089 is `RETIRED` with a reason, not a `RESCOPED` ghost; the next planner who reads the post-RE-PLAN-008 plan sees the production-path empty-input contract is shipped and does not re-promote a slice for it).**
## Next-phase active items (RE-PLAN-009 2026-06-10 by designcritic, RE-PLAN task t_e00cbf38; complements RE-PLAN-004 / RE-PLAN-005 / RE-PLAN-006 / RE-PLAN-007 / RE-PLAN-008 — does NOT retread any of the 22 prior rows)

The post-RE-PLAN-008 plan is structurally close to "exhausted" but the *immediate* next slice (commit STW-091's working-tree draft) is not a planning exercise — it is a single integrated commit a worker can land in 1 hour. RE-PLAN-009 names that slice, retires the 3 stale `[ ]` ghost rows RE-PLAN-008 left behind (the gate currently fails with 2 ghosts: STW-088, STW-089; STW-090 is the 3rd ghost the body says is RETIRED but the checkbox still says `[ ]`), and re-issues 2 carried-over rows (STW-076 dashboard/receipt adapter, STW-074 operator-decisions) with the truthful dependencies the receipt chain has surfaced.

What changed in the 4 hours between RE-PLAN-008 and RE-PLAN-009 (audit at 18:21 UTC, 2026-06-10):

- **A worker is mid-implementation of STW-091 in the working tree.** `git diff --stat` shows 9 files modified, 283 insertions, 11 deletions: the `Layer::lookup` cap (`layer.rs:79-140` adds `synthetic_for_test` constructor + `lookup_prefix` + `lookup_with_prefix` entry point), the `RBP_FAST_LOOKUP_SAMPLE` env helper + `FAST_LOOKUP_SAMPLE_DEFAULT` const in `crates/util/src/lib.rs` (mirroring the kmeans `fast_kmeans_sample` + `FAST_KMEANS_SAMPLE_DEFAULT` pattern), a new `crates/clustering/tests/lookup_fast.rs` integration test file (~15 KB, 3 sub-tests per the RE-PLAN-008 spec: `lookup_fast_caps_sample_at_1024` + `lookup_production_unchanged_when_fast_unset` + `lookup_does_not_panic_on_underdetermined_input`), the runbook + doc updates (`scripts/testnet-live-proof.sh` gains the `RBP_FAST_LOOKUP_SAMPLE` auto-set line + the `ENV.txt` summary print; `scripts/testnet-live-proof.md` gains the env-knob table row; `crates/autotrain/tests/script_shape.rs` extends the `runbook_doc_lists_every_env_knob` pin), and a `crates/clustering/src/lookup.rs` cosmetic edit (the `(row.get...)` triple-tuple re-wrapped on 5 lines instead of 1 — a rustfmt whitespace drift, not a semantic change). **The STW-091 spec is structurally complete in the working tree** (the cap, the env helper, the test, the runbook updates all match the RE-PLAN-008 acceptance criteria). The working tree is NOT yet a clean `git status` — it has the STW-091 work PLUS the half-drafted STW-088 property-test stub (the `kmeans_property.rs` 2-of-5-property stub + the `proptest = "1.4"` dev-dep + the `pub fn init_kmeans_plus_plus` exposure + the `.proptest-regressions` cache) the RE-PLAN-008 STW-092 row already names.
- **`bash scripts/plan-staleness-gate.sh` currently FAILS with 2 ghosts** (STW-088 at line 11612 + STW-089 at line 11614, both `[ ] [P1]` rows in RE-PLAN-007 form that RE-PLAN-008 RETIRED as `[x]` at lines 11658 + 11660 — the `RESCOPED <date> by RE-PLAN-N` convention the gate's P1 re-issue branch looks for is NOT in the `[x]` rows' "RETIRED" markers, so the gate counts a ghost). The gate is the mechanical enforcement of the planner's promise; a planning cycle that leaves the gate in FAIL is incomplete. RE-PLAN-009 closes the leak.
- **STW-090 is the 3rd ghost** the gate does not catch (the gate's P1 pass scans only `[ ] [P1]` rows, and STW-090 is `[ ] [P0]`) but the plan-vs-reality surface is the same: line 11616 says `STW-090 is RETIRED without re-issuing in RE-PLAN-007 form` in the body but the checkbox is `[ ]`. RE-PLAN-009 flips the checkbox to `[x]` to honor the body.
- **The receipt chain still has 33 failed/partial `receipts/testnet-live-proof-2026060{4..10}T*` directories** in `receipts/` (the 2026-06-10 11:20 `cluster=hang at calculating_lookup_flop` is the most recent; the others are kmeans-panic, runbook-shape, db-reachable, etc.). The archive work STW-085 from RE-PLAN-005 described is *still* un-landed; STW-093 from RE-PLAN-008 promotes STW-085 from P1 to P0-dependency for the right reason — the receipts dir is a graveyard the runbook cannot produce a clean "one green + 0 stale" state without first cleaning up.
- **The 2 carried-over P1 rows (STW-076 dashboard-receipt-route, STW-074 operator-decisions) are unchanged from RE-PLAN-002/003/004**. They have been un-claimed for 5 RE-PLAN cycles (RE-PLAN-002 → RE-PLAN-003 → RE-PLAN-004 → RE-PLAN-005 → RE-PLAN-006 → RE-PLAN-007 → RE-PLAN-008); RE-PLAN-009 re-issues them with the truthful dependencies the receipt chain has surfaced and the worker pool the active queue currently has.

What RE-PLAN-009 changes versus RE-PLAN-008:

- **3 row-retirement flips** (STW-088 + STW-089 + STW-090 → `[x]`), in the existing RE-PLAN-007 section, to honor the bodies' "RETIRED" markers. The retirement rows at lines 11658 + 11660 + 11616 are the audit trail; flipping the source `[ ]` rows to `[x]` makes the gate green.
- **STW-094 (NEW, P0) — `COMMIT-STW-091-CAP-AND-CLOSE-WORKING-TREE`** — the *integrated* commit that lands the STW-091 lookup-cap code (already in the working tree) + closes the half-drafted STW-088 working-tree state. The worker picks the (a)-commit-STW-088-stub-or-(b)-revert-and-re-issue branch (per the RE-PLAN-008 STW-092 spec) and the commit lands on `main` with the working tree clean. This is the immediate next slice — a single commit a worker can land in 1 hour.
- **STW-095 (NEW, P0) — `RUN-TESTNET-LIVE-PROOF-POST-STW-091`** — the *terminal evidence* slice re-issued under a new id (it absorbs STW-093 from RE-PLAN-008) with the truthful dependency chain `STW-075 + STW-077 + STW-078 + STW-086 + STW-087 + STW-094` and the integrated STW-085 archive work (move 33 failed receipts to `receipts/_archive/failed/` + write `INDEX.md` + add `!receipts/_archive/` to `.gitignore` + bump the receipt-dir naming to `_v3` suffix). The fresh-DB runbook run after STW-094 lands reaches the `bench` step in well under 5 minutes per street (the dominant per-street cost is now the 1024-row lookup, not the 1.3M-row lookup) and lands a green `receipts/testnet-live-proof-<UTC>_v3/` directory the runbook commits.
- **STW-096 (NEW, P1) — `DASHBOARD-LATEST-RECEIPT-ROUTE` (RE-PLAN-009 re-issue of STW-076 from RE-PLAN-004)** — close the v10 → v11 gap: a `GET /api/receipt/latest` route on the dashboard that reads from `RBP_DASHBOARD_RECEIPTS_DIR` (default `../receipts`) + serves the latest committed `testnet-live-proof-<UTC>_v3/` receipt's `SUMMARY.txt` + `recipe.json` + per-step `stdout.txt` / `exit.txt` as a typed JSON envelope, with a fixture-fallback (`crates/autotrain/tests/fixtures/testnet-live-proof-fixture/`) for when no real receipt exists. A stranger `curl`-ing the deployed dashboard after STW-095 + STW-096 lands sees the green receipt from STW-095 (or the fixture fallback) — the v10 → v11 adapter closes the "stranger has to `git clone`" gap RE-PLAN-003's STW-076 row named.
- **STW-097 (NEW, P1) — `OPERATOR-DECISIONS` (RE-PLAN-009 re-issue of STW-074 from RE-PLAN-002 / RE-PLAN-003 / RE-PLAN-004)** — close the 2 `[!]` operator-decision rows (`STW-001` planning surface, `STW-007` artifact retirement) with a recorded verdict + a new `steward/ARTIFACT-RETENTION.md` per-path verdict table. **The only row in RE-PLAN-009 that genuinely blocks on human input** (the `.gbrain-source` / `.auto/tui*/` / `.auto/orchestrator/velocity-*` / `.auto/corpus-staging/` / `.auto/logs/steward-*-prompt.md` retention verdicts); every other row is a code/markdown change a worker can ship solo. Re-issued with the post-STW-091/092/093/094/095/096 dependency (the `.auto/tui*/` retention verdict is easier after the receipt chain is green because the receipts dir is no longer a graveyard).
- **The 4 RE-PLAN-004/005 P0 carry-overs (STW-080 + STW-081 + STW-082 + STW-083) and the 2 RE-PLAN-004/005 P1 carry-overs (STW-084 + STW-085 [now P0-dependency of STW-095]) and the 3 RE-PLAN-004/005/006 carry-overs (STW-071 + STW-074 → STW-097, STW-076 → STW-096, STW-079) carry over unchanged.** STW-071's 13 ghost P1 rows (STW-040, 041, 045, 046, 048, 056, 057, 060, 063, 064, 066, 068) are *not* in scope for RE-PLAN-009 — the gate's P1 pass does not flag them as ghosts (the gate only catches `[ ] [P1]` rows that have a corresponding `[x] STW-NNN` row; the 13 ghost rows have a `[x] STW-NNN` row only in the sense that the `[x]` REPLACEMENT is a different STW id, not the same one — STW-040's body says "DROPPED 2026-06-04 by STW-046" but the gate doesn't parse body DROPPED markers). The 13 ghost rows are *de facto* retirements that the gate's current logic does not mechanically catch; STW-071 is the markdown slice that retires them. A future RE-PLAN that wants the gate to mechanically catch body-DROPPED markers is a small bash-script extension (~5 lines + 1 sub-test).
- **No new P0 rows beyond STW-094 + STW-095.** The 5 P0 dependencies STW-093 (now STW-095) names are all shipped (`STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087`); the only remaining work is the STW-091 lookup-cap commit (STW-094) + the receipt-chain run (STW-095).
- **The 9 P0/P1 carry-overs from RE-PLAN-004/005** (STW-071 markdown-retirement, STW-079 HINGE-CLOSEOUT-SEAT-PERSIST-001, STW-080 server http-integration-tests, STW-081 ci-runs-integration-tests, STW-082 server-deploy-runbook, STW-083 api-docs-and-curl-examples, STW-084 fast-mode-parity, STW-085 failed-receipt-archive [now P0-dependency of STW-095], STW-086 kmeans-empty-histogram-guard [x] SHIPPED) **carry over unchanged**. They are real, claimable, independent of the receipt loop, and have not been retreaded.

The 4 new rows below (STW-094 P0 + STW-095 P0 + STW-096 P1 + STW-097 P1) plus 3 checkbox-flips (STW-088/089/090 → `[x]` in the existing RE-PLAN-007 section) are the minimum surface that closes the planning cycle from "RE-PLAN-008 leaves the gate in FAIL + 3 ghost rows in the active queue" to "RE-PLAN-009 is green: 1 commit closes the working tree + 1 commit runs the receipt + 1 commit ships the dashboard adapter + 1 markdown-only slice records the operator decisions, with the gate at `checked=N ghosts=0`". **Active open queue count post-RE-PLAN-009:** RE-PLAN-004 leaves 2 open rows (STW-071 + STW-079; STW-070 was RETIRED, STW-072 is DEPRECATED, STW-073 is DEFERRED, STW-074 → STW-097, STW-076 → STW-096). RE-PLAN-005 leaves 5 open rows (STW-080 + STW-081 + STW-082 + STW-083 + STW-084; STW-085 is now a P0-dependency of STW-095, not a standalone P1). RE-PLAN-006 leaves 0 open rows (STW-086 is [x] SHIPPED). RE-PLAN-007 leaves 0 open rows (STW-087 is [x] SHIPPED; STW-088 + STW-089 + STW-090 are [x] RETIRED in RE-PLAN-009 via checkbox-flip). RE-PLAN-008 leaves 2 open rows (STW-091 → STW-094, STW-092 → folded into STW-094, STW-093 → STW-095). RE-PLAN-009 adds 4 new rows (STW-094 + STW-095 + STW-096 + STW-097). **Total post-RE-PLAN-009 active open `[ ]` rows: 11** (2 RE-PLAN-004 + 5 RE-PLAN-005 + 4 RE-PLAN-009 - the 1 row for STW-091 is folded into STW-094). The 13 ghost P1 rows from RE-PLAN-002's STW-071 spec (STW-040, 041, 045, 046, 048, 056, 057, 060, 063, 064, 066, 068) are also `[ ]` but are RESCOPED/DROPPED noise; STW-071 itself retires them in a follow-on slice. A `rg -n "^- \[ \] \*\*\[P[01]\]" IMPLEMENTATION_PLAN.md` after this commit shows STW-071, STW-079 (RE-PLAN-004) + STW-080, STW-081, STW-082, STW-083, STW-084 (RE-PLAN-005) + STW-094, STW-095, STW-096, STW-097 (RE-PLAN-009). Priority order: **STW-094 is the smallest P0 with the highest leverage** (it is the commit that closes the 18-hour-old working-tree state and unblocks STW-095); **STW-095 is the terminal evidence slice** (the green receipt the testnet north star demands); **STW-096 is the v10 → v11 adapter** (the first slice a stranger can `curl`); **STW-097 is the operator-decision closeout** (the only row that needs human input). Owner for the RE-PLAN row itself: designcritic, 2026-06-10.

- [ ] **[P0] `STW-094` `COMMIT-STW-091-CAP-AND-CLOSE-WORKING-TREE` (NEW in RE-PLAN-009; absorbs STW-091 + STW-092 from RE-PLAN-008 into a single integrated commit) — the 2026-06-10 18:21 working tree carries the STW-091 lookup-cap code (structurally complete per the RE-PLAN-008 spec) PLUS the half-drafted STW-088 property-test stub (the `Cargo.lock` + `crates/clustering/Cargo.toml` `proptest = "1.4"` dev-dep + the `crates/clustering/src/kmeans.rs` `pub fn init_kmeans_plus_plus` exposure + the untracked `crates/clustering/tests/kmeans_property.rs` 2-of-5-property stub with 3 `todo!()`s + the untracked `kmeans_property.proptest-regressions` cache). STW-094 closes the working-tree-vs-plan drift in a single integrated commit that lands on `main` as `git status` clean. The commit is the STW-091 work + the STW-092 work, decided as a single decision the worker makes after reading the working tree. **Two acceptance paths, both individually shippable, both end with `git status` clean:** (path-a, "ship the stub") — the worker reads the 2 drafted `kmeans_property.rs` property sub-tests (`run_fast_no_panic_on_random_input` + the second drafted one) + judges them shippable (they compile green, exercise a real no-panic contract, and the 3 `todo!()` stubs are non-blocking failures the next RE-PLAN's `STW-088b` re-issue will fill in) + commits the working tree as a single `feat(clustering): STW-091 lookup fast-mode cap + STW-088 property-test layer (2-of-5 properties + dev-dep + visibility exposure)` commit. The commit body names `STW-094` as the integrated slice + the `STW-088b RE-PLAN-009 re-issue` note for the 3 `todo!()` properties + the `kmeans_property.proptest-regressions` cache. **Path-b, "revert the stub") — the worker reads the 2 drafted property sub-tests + judges them NOT shippable (e.g. they don't compile, they test the wrong contract, or the worker judges the half-drafted stub is more confusing than helpful) + reverts ONLY the STW-088 working-tree changes (`git checkout -- Cargo.lock crates/clustering/Cargo.toml crates/clustering/src/kmeans.rs && rm crates/clustering/tests/kmeans_property.rs crates/clustering/tests/kmeans_property.proptest-regressions`) + commits ONLY the STW-091 work as `feat(clustering): STW-091 fast-mode lookup sample cap in Layer::lookup` (the same commit title the RE-PLAN-008 STW-091 spec names). The commit body names `STW-094` + the `STW-088` re-issue spec the RE-PLAN-009 `STW-088b` row will name + a one-paragraph audit-trail note in `steward/WORKING-TREE-DECISIONS.md` the path-b branch requires. **The path-a / path-b decision is the worker's** (the RE-PLAN-009 STW-094 spec is intentionally narrow — it does not make the decision for the planner). The acceptance criteria for the integrated commit are: `git status` is clean post-commit; `cargo test -p rbp-clustering --test lookup_fast` is green with 3 new sub-tests (the `lookup_fast_caps_sample_at_1024` + `lookup_production_unchanged_when_fast_unset` + `lookup_does_not_panic_on_underdetermined_input` sub-tests the RE-PLAN-008 STW-091 spec names); `cargo test -p rbp-clustering --test kmeans_fast` stays green (no regression in STW-077/086/087's 5 sub-tests); `cargo test -p rbp-clustering --lib` stays green; `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with `plan staleness gate complete: checked=N ghosts=0` (the RE-PLAN-009 row-flips close the 2 STW-088 + STW-089 ghosts; STW-094's commit does not re-introduce them). For path-b specifically: `steward/WORKING-TREE-DECISIONS.md` is committed with a one-paragraph audit trail (the (a)/(b) decision + the reason + the date + the `STW-088` re-issue spec); the `Cargo.toml` is reverted to the pre-STW-088 `proptest` dev-dep state; the `kmeans.rs` is reverted to the pre-STW-088 `pub` exposure. For path-a specifically: `cargo test -p rbp-clustering --test kmeans_property` is green with 2 sub-tests passing + 3 `todo!()`s (the `todo!()`s are a known-failing-tests pin, not a regression — the 2 green sub-tests are the proof the framework is wired correctly); the `proptest-regressions` cache is committed (a 1-line file proptest writes automatically). **Scope boundary:** do NOT change the `Layer::lookup` semantics (the cap is a fast-mode-only path mirroring the existing `Layer::cluster` fast-mode gate at `layer.rs:225-242`; the production path is byte-identical when `RBP_TESTNET_FAST` is unset); do NOT change the kmeans driver (the `run_fast` / `init_kmeans_plus_plus` paths are unchanged); do NOT change the `N_FLOP` / `N_TURN` / `N_PREF` / `N_RIVE` consts; do NOT change the `Street::Pref | Street::Rive` arm of `Layer::lookup`; do NOT change the `calculating metric` or `calculating transitions` steps; do NOT change the runbook's chain step order; do NOT change the `LiveProofReceipt::read_and_verify` verifier; do NOT touch any v1/v2/v3 profile schema, the dashboard, the publish/index/remote chain, the seat-aware work, the autotrain pipeline, or any `trainer --*` CLI; do NOT re-touch the v6 → v10 follow-on chain narrative earlier in the plan. **Dependencies:** the working tree's STW-091 code (already drafted — the worker ports + decides + commits); the working tree's STW-088 stub (already drafted — the worker judges shippable-or-not); the existing `crates/clustering/src/kmeans.rs` (the `run_fast` API STW-088's property test targets); the existing `crates/clustering/src/layer.rs` (the `init_kmeans` + the new `Layer::lookup` cap STW-091 adds). **Estimated scope:** S (one commit + ~30 lines of new code in `layer.rs` + ~150 lines of new tests in `lookup_fast.rs` + the path-b audit-trail file if path-b is chosen, OR the path-a stub + dev-dep state if path-a is chosen). **Completion signal:** `git log -1 --oneline` on `main` shows a new commit titled either `feat(clustering): STW-091 lookup fast-mode cap + STW-088 property-test layer (2-of-5 properties + dev-dep + visibility exposure)` (path-a) or `feat(clustering): STW-091 fast-mode lookup sample cap in Layer::lookup` (path-b); `git status` is clean post-commit; a fresh `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` no longer hangs at `calculating lookup flop` (the receipt's `cluster/stdout.txt` shows the STW-091 cap's `lookup sample=1024 (production would be N=1286792)` log line + the `cluster/exit.txt` is `0`, not `101`); `bash scripts/plan-staleness-gate.sh` exits 0 with `plan staleness gate complete: checked=N ghosts=0`. **`lens:` CEO (the v1→v10 testnet chain is structurally complete; the only remaining work is the STW-091 lookup cap that RE-PLAN-008 diagnosed + the working-tree discipline that RE-PLAN-008's STW-092 spec named; STW-094 is the *integrated* commit that ships both in one swing — the next-phase discipline is to converge the working tree, not to add a new slice) + Eng (the STW-091 work is structurally complete in the working tree; the worker ports + decides + commits; the path-a / path-b decision is narrow and the audit trail is committed either way) + Design (the integrated commit lands on `main` with a clean `git status`; a planner opening the plan + the working tree after STW-094 sees a single source of truth, not a half-drafted stub + 1 real slice).**

- [ ] **[P0] `STW-095` `TESTNET-LIVE-PROOF-RECEIPT-POST-STW-091` (NEW in RE-PLAN-009; absorbs STW-093 from RE-PLAN-008 + the STW-085 archive work from RE-PLAN-005) — run `scripts/testnet-live-proof.sh` end-to-end with `RBP_TESTNET_FAST=1` against a fresh STW-078-provisioned Postgres, with `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` all shipped, and commit the first green `receipts/testnet-live-proof-<UTC-THIS-WEEK>_v3/` directory the receipt chain has produced. The `STW-093` row's RE-PLAN-008 dependency chain (\"depends on `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-091`\") is correct in spirit but its STW-091 reference is now a STW-094 reference (STW-091 is folded into STW-094 in RE-PLAN-009); the truthful dependency chain is `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094`, and STW-095 is the slice that runs the runbook end-to-end post-STW-094.** This is the *terminal evidence* slice: it converts the v1→v10 testnet chain from "structurally complete, kmeans-panic-blocked AND lookup-hang-blocked" to "operationally proven." The runbook's 8 step exits (`doctor` / `cluster` / `reset` / `smoke` / `status` / `bench` / `compare` / `replay`) must all be `0`; the `SUMMARY.txt` headline must be the pinned `testnet live_proof complete: smoke=N status=N bench=N compare=N replay=BYTES` line; the `recipe.json` must re-verify with `LiveProofReceipt::read_and_verify`; the receipt is committed on `main`. STW-095 is *evidence only*: the runbook is run, the receipt is captured, the verifier is invoked, the result is committed. If a fresh runbook run fails after STW-094 lands, the worker reports the failure as a new `[P0]`-ranked next slice (or blocks for human input) rather than silently fixing the runbook in the STW-095 commit. STW-095 *also* executes the STW-085 archive work RE-PLAN-005 described (RE-PLAN-009 promotes STW-085 from P1 to P0-dependency of STW-095 because the receipts dir is currently a graveyard): move the 33 failed/partial `receipts/testnet-live-proof-2026060{4..10}T*` directories into `receipts/_archive/failed/` (operator-side, the move preserves the git-ignored receipts state) + write a `receipts/_archive/failed/INDEX.md` (committed, ~50 lines; one bullet per archived receipt + the failure mode + the slice that fixed it: e.g. `20260610T112026Z: cluster-hang at calculating_lookup_flop, fixed by STW-091 lookup fast-mode cap`, `20260610T032421Z: cluster=panic at bins.rs:95:31 during Lloyd step, fixed by STW-087 empty-cluster guard`, `20260610T001633Z: cluster=panic at bins.rs:95 during kmeans++ init, fixed by STW-086 empty-histogram pre-filter`, `20260610T013757Z: cluster=panic at bins.rs:95 during kmeans++ init, fixed by STW-086 empty-histogram pre-filter`, `20260609T042107Z: cluster=hang at kmeans iterating on turn, fixed by STW-077 fast-mode sample/iteration cap`, `20260609T060233Z: doctor=2 db_reachable=false, fixed by STW-078 postgres-env provisioning`, `20260608T225611Z..20260608T235657Z (4 receipts): cluster=101 SIGTERM timeout mid-iteration, fixed by STW-075 deterministic Check::clustered + STW-077 fast-mode cap`, `20260608T184917Z: cluster=panic, fixed by STW-075 + STW-086`, `20260608T170211Z..20260608T165906Z (2 receipts): cluster=panic + lookup-hang, fixed by STW-075 + STW-086 + STW-087 + STW-091`, `20260608T110812Z..20260608T135801Z (2 receipts): cluster=panic, fixed by STW-075`, `20260606T002154Z..20260606T010851Z (2 receipts): cluster=panic, fixed by STW-075`, `20260606T002116Z: cluster=panic, fixed by STW-075`, `20260605T191353Z: cluster=panic, fixed by STW-075`, `20260605T185127Z: cluster=panic, fixed by STW-075`, `20260604T140107Z..20260604T164243Z (7 receipts): cluster=panic + runbook-shape drift, fixed by STW-075 + STW-019 + STW-022` — the INDEX.md is a real audit trail a future planner can read to understand the failure history without re-running the 33 failed receipts) + update `.gitignore` to add `!receipts/_archive/` so the archive dir is the *one* thing tracked (the operator-visible audit trail of "what went wrong" is now in the repo, not just on a local disk; the `_archive/` dir is the audit-trail the README's `## Testnet launch proof` section can link to) + update `scripts/testnet-live-proof.sh`'s receipt-dir naming convention to append a `_v3` suffix on the live `receipts/` dir (e.g. `receipts/testnet-live-proof-<UTC>_v3/`) so a future receipt never collides with the archived ones (mirroring the STW-085 spec's `_v2` suffix but bumped to `_v3` because the receipt shape has been updated by the `RBP_FAST_LOOKUP_SAMPLE` knob the runbook now exports) + update `scripts/testnet-live-proof.md`'s `Receipts layout` section to document the new `receipts/_archive/failed/` location + the `INDEX.md` summary + the `_v3` suffix. The receipt's `cluster/stdout.txt` is the operator-visible proof the STW-094 lookup-cap holds in production (a fresh `lookup sample=1024 (production would be N=1286792)` log line + a `cluster=0` exit is the receipt-side verification of the STW-094 fix). **Owner files:** `scripts/testnet-live-proof.sh` (no code change expected beyond the `_v2` → `_v3` suffix bump; the `RBP_FAST_LOOKUP_SAMPLE` knob is set by STW-094's runbook-update), `scripts/testnet-live-proof.md` (note the evidence path + the `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` dependency chain + the `_v3` suffix + the `receipts/_archive/failed/` location), the new green receipt directory under `receipts/testnet-live-proof-<UTC>_v3/` (the deliverable), `receipts/_archive/failed/INDEX.md` (new — the failure-history summary, ~50 lines; one bullet per archived receipt + the failure mode + the slice that fixed it), `.gitignore` (add `!receipts/_archive/` to the existing `/receipts/` ignore line per the STW-085 spec), `steward/HAZARDS.md` (the `TESTNET-LIVE-PROOF-RECEIPT` row flips from "open" to "closed by STW-095 on <date>" once the receipt is committed), `steward/DRIFT.md` (the `STW-019` row's "DRIFT" verdict on the receipts-orphans line flips to "RESOLVED by STW-095 on <date>" once the new receipt is committed; the `STW-085` row flips from "open" to "closed by STW-095 on <date>" once the archive + INDEX are committed). **Scope boundary:** do NOT change the runbook's chain step order; do NOT weaken `LiveProofReceipt::read_and_verify`; do NOT bypass `trainer --doctor`; do NOT touch the `Check::clustered` fix (that is `STW-075`'s commit, not this one); do NOT touch the kmeans cap (that is `STW-077`'s commit, not this one); do NOT touch the postgres provisioning script (that is `STW-078`'s commit, not this one); do NOT touch the kmeans empty-histogram pre-filter (that is `STW-086`'s commit, not this one); do NOT touch the kmeans empty-cluster guard (that is `STW-087`'s commit, not this one); do NOT touch the kmeans lookup fast-mode cap (that is `STW-094`'s commit, not this one); do NOT touch the working-tree discipline slice (that is `STW-094`'s commit, not this one); do NOT touch the dashboard / publish / index / index-remote / remote chain; do NOT delete the existing failed/partial `receipts/testnet-live-proof-*` directories (they are archived, not deleted); do NOT change the `RBP_FAST_LOOKUP_SAMPLE` default (1024, set by STW-094; STW-095 honors the default). **Acceptance criteria:** a `find receipts/testnet-live-proof-<UTC>_v3/` shows 8 step subdirs each with `stdout.txt` + `stderr.txt` + `exit.txt` where every `exit.txt` is `0`; `SUMMARY.txt` head is the pinned `testnet live_proof complete: ...` line; `recipe.json` parses with `LiveProofRecipe`; `trainer --verify-receipt receipts/testnet-live-proof-<UTC>_v3/` exits 0 and prints `live_proof receipt verification passed: ...`; the receipt is committed on `main`; the `receipts/_archive/failed/` directory contains all 33 archived receipts + the `INDEX.md` summary; `git status` is clean post-commit; `steward/HAZARDS.md` + `steward/DRIFT.md` + `steward/PROMOTIONS.md` reflect the closed `TESTNET-LIVE-PROOF-RECEIPT` hinge + the closed `STW-085` archive row. **Dependencies (RE-PLAN-009):** `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` + a reachable `DATABASE_URL` or `DB_URL`; existing `STW-067` (fast-mode) + `STW-069` (fail-before-train integrity gate) + `STW-019` (runbook) + `STW-023` (shared verifier + `recipe.json`) + `STW-028` (`--verify-receipt` CLI) + `STW-085` (the FAILED-RECEIPT-ARCHIVE spec STW-095 integrates + promotes from RE-PLAN-005 P1 to STW-095 P0-dependency). **Estimated scope:** S (operator-runnable evidence, on a fresh DB, post-`STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094`; the receipt capture + the archive move + the INDEX.md write + the steward updates = 1 integrated commit, not 4). **Completion signal:** a fresh `trainer --verify-receipt` exits 0 on a `receipts/testnet-live-proof-<UTC-THIS-WEEK>_v3/` directory and the receipt path is recorded in `steward/HAZARDS.md` row #2 + `steward/DRIFT.md` `STW-019` row as the current operator proof; the receipts dir shows only the new green receipt + the `_archive/failed/` summary; the `verification:workspace-parallel` hinge #2 is now `closed by STW-095 on <date>` with the receipt path as evidence; the next planner pass sees a `RE-PLAN-010` task with no `STW-070` / `STW-088` / `STW-089` / `STW-090` / `STW-091` / `STW-092` / `STW-093` / `STW-094` / `STW-095` carry-over (the receipt chain is *evidenced*; the planner can move to a different north star — the v10 → v11 dashboard/receipt adapter (STW-096, the next slice after STW-095), the seat-aware slice 5 export (STW-073), the operator-decision closeout (STW-097), the server / CI / public-surface hardening (STW-080/081/082/083/084), or the public testnet claim follow-on). **`lens:` CEO (the testnet claim requires a green receipt; STW-095 is the slice that converts "structurally complete" to "operationally proven" with the *correct* dependency chain — the 6 RE-PLAN cycles of STW-070 re-issues have all been correct in spirit (the chain is structurally complete) but wrong in detail (each re-issue missed a layer: the kmeans cap was the wrong layer for the 2026-06-10 11:20 hang, the lookup cap is the right layer, STW-094 lands the lookup cap, STW-095 runs the receipt) + Eng (the runbook is run, the receipt is captured, the verifier is invoked, the 33 failed receipts are archived, the INDEX.md is written, the steward is updated — a single integrated commit, the same shape the STW-085 spec described, now elevated to P0 because the receipts dir is currently a graveyard) + Design (the receipts dir a stranger `ls`-es shows the *honest* state: one green `_v3` receipt + one archived failure-history summary with 33 bullets + the `_v3` suffix, not 33 confusing stale dirs + 0 green ones).**

- [ ] **[P1] `STW-096` `DASHBOARD-LATEST-RECEIPT-ROUTE` (NEW in RE-PLAN-009; absorbs STW-076 from RE-PLAN-004 / RE-PLAN-003) — close the v10 → v11 gap: a thin `GET /api/receipt/latest` (and `GET /api/receipt/<basename>`) route on the existing `crates/dashboard/` `axum` router that reads from a configured `RBP_DASHBOARD_RECEIPTS_DIR` (default `../receipts`, relative to the dashboard crate) + serves a single typed JSON envelope `{ basename, summary, recipe, steps, source: "receipt" | "fixture" }` to a stranger who `curl`s the deployed dashboard. After STW-095 lands, the receipt the runbook commits to `receipts/testnet-live-proof-<UTC>_v3/` is the *primary operator-visible proof* of the testnet north star; without a dashboard route, a stranger has to `git clone` to read it. The envelope re-uses the `LiveProofRecipe` struct the autotrain crate already ships; the new code is a thin file-read + `serde_json::from_str` wrapper, no new domain types. The `source` field disambiguates the two surfaces: a real receipt from `RBP_DASHBOARD_RECEIPTS_DIR` reports `source: "receipt"`; a fallback to the committed `crates/autotrain/tests/fixtures/testnet-live-proof-fixture/` reports `source: "fixture"`. The fallback engages *only* when no real receipt exists (i.e. `RBP_DASHBOARD_RECEIPTS_DIR` is empty / missing / or contains no `testnet-live-proof-<UTC>_v3/` subdirectories); the integration test asserts the fallback path is gated on real-receipt absence so a future receipt never gets papered over. The `latest` route picks the lexicographically-maximum `testnet-live-proof-<UTC>_v3/` basename from the real-receipt dir (the same UTC-ISO sort the runbook's `SUMMARY.txt` line `testnet live_proof complete: ...` follows, so dashboard sort matches receipt sort); if no real receipt exists, the route returns the committed fixture's envelope with `source: "fixture"`. The `<basename>` route only matches real-receipt basenames (the fixture has no `<basename>` route — a request for `testnet-live-proof-fixture` returns 404 with `{"error":"receipt_missing","basename":"testnet-live-proof-fixture"}` so the fallback path is *only* reachable via the `latest` route). Both routes return `404 + a typed DashboardError JSON body` when no match is found (the dashboard's existing `crates/dashboard/src/router.rs` error mapper renders the typed body the same way the existing `IndexClient::read` / `IndexMissing` paths do, so a stranger can `curl .../api/receipt/latest` and `jq .error` for the failure shape). The static `crates/dashboard/static/index.html` empty-state gets a one-line `<a href="/api/receipt/latest">Latest live_proof receipt (JSON)</a>` link beside the existing "Demo data: `/bench/compare3-fixture`" link so a first-time visitor can audit the most-recent receipt (or the fixture fallback) without writing `curl + jq`. A new `crates/dashboard/tests/receipt_route.rs` integration test drives the axum router end-to-end and asserts: (a) `GET /api/receipt/latest` on a temp `RBP_DASHBOARD_RECEIPTS_DIR` containing one synthetic `testnet-live-proof-20260608T210000Z_v3/` returns 200 with the pinned envelope shape — `source: "receipt"`, `summary` matches `SUMMARY.txt` verbatim, `recipe.steps[0].name == "doctor"`, `steps[0].exit == 0`; (b) `GET /api/receipt/testnet-live-proof-20260608T210000Z_v3` returns the same envelope; (c) `GET /api/receipt/does-not-exist` returns 404 + `{"error":"receipt_missing","basename":"does-not-exist"}`; (d) `GET /api/receipt/latest` on an empty / missing `RBP_DASHBOARD_RECEIPTS_DIR` falls back to the committed `testnet-live-proof-fixture` and returns 200 with `source: "fixture"`; (e) `GET /api/receipt/testnet-live-proof-fixture` returns 404. The `serve_receipt` handler is a small file-read function (< 100 lines, no tokio-postgres, no JSON streaming, no new dep — only `serde_json` + `std::fs::read_to_string` + `axum::Json`); the existing `OnceLock<String, String>` + mtime-invalidation pattern the STW-062 / STW-063 cache already follows is reused for the `<basename>` route so a receipt re-read is a cache hit on the second `curl` (the `latest` route is uncached by design — its sort order is the operator-visible signal, and the fixture-fallback is a static read so caching it would also be correct, but a consistent "latest is always live" posture is friendlier to ops). **Owner files:** `crates/dashboard/src/router.rs` (new `serve_receipt_latest` + `serve_receipt_named` handlers + `DashboardError::ReceiptMissing(<basename>)` + `DashboardError::ReceiptsDirMissing(<path>)` + `DashboardError::FixtureMissing(<path>)` variants; reuse the existing typed-error JSON mapper the `IndexClient::read` path already follows), `crates/dashboard/src/state.rs` (the `RBP_DASHBOARD_RECEIPTS_DIR` env knob read at boot — default `"../receipts"` so a `cargo run -p rbp-dashboard` from the workspace root resolves to the in-repo `receipts/` dir; absolute paths also supported; the committed `crates/autotrain/tests/fixtures/testnet-live-proof-fixture/` path is hard-coded as the fallback), `crates/dashboard/static/index.html` (one-line `<a href="/api/receipt/latest">...</a>` link in the existing empty-state paragraph), `crates/dashboard/tests/receipt_route.rs` (new no-DB integration test file with the 5 sub-tests above), `scripts/deploy-dashboard-cloudflare.md` (note the `RBP_DASHBOARD_RECEIPTS_DIR` knob the deploy runbook should set to a path Cloudflare Pages can read at request time — a Pages Functions rewrite can serve the route from the same `wrangler pages deploy` artifact the dashboard already ships), `IMPLEMENTATION_PLAN.md` (this row), `genesis/plans/000-ceo-testnet-roadmap.md` (mark the v10 follow-on-the-follow-on as shipped with a one-line note). **Scope boundary:** do NOT change the `INDEX.json` / `IndexClient` / bench-card path (the v10 deployed surface is unchanged — the receipt route is an *additional* surface, not a replacement); do NOT change the `trainer --verify-receipt` CLI (the dashboard route is a *typed read* of the on-disk receipt, not a re-verify — a downstream auditor who needs the re-verify path still uses `trainer --verify-receipt`); do NOT push the receipt to S3 / Cloudflare R2 (the dashboard reads the receipt from the same `wrangler pages deploy` artifact or a Pages Functions read-through to the deploy-time-mounted path — a future CI-worker slice can `wrangler r2 object put` the receipt in a follow-on); do NOT change the receipt's on-disk layout (the route reads what `scripts/testnet-live-proof.sh` wrote — the runbook + the route share one shape); do NOT touch any v1/v2/v3 profile schema, the seat-aware work, the publish/index/remote chain, the v6 → v10 follow-on chain, the v1-v2-v3 trained-config axis, the `SEAT-PERSIST-001` hinge, or the `verification:workspace-parallel` hinge. STW-096 is the *adapter* between the offline operator evidence (the receipt directory the runbook commits) and the public deployed surface (the dashboard URL the README links) — it does not change either side, it adds a typed read of the former through the latter. **Acceptance criteria:** `cargo test -p rbp-dashboard --test receipt_route` is green with 5 new sub-tests; `cargo test -p rbp-dashboard --lib` stays green; `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts. **Hand-test:** after STW-095 + STW-096 both land, a `cargo run -p rbp-dashboard` + a `curl http://localhost:8080/api/receipt/latest` returns 200 with the green receipt from STW-095's `receipts/testnet-live-proof-<UTC>_v3/` (source: "receipt") or the committed fixture (source: "fixture") if no green receipt exists; a `curl http://localhost:8080/api/receipt/does-not-exist` returns 404 with `{"error":"receipt_missing","basename":"does-not-exist"}`. **Dependencies:** the existing `crates/dashboard/src/router.rs` (the typed-error mapper is the model); the existing `crates/autotrain/src/receipt.rs` (the `LiveProofRecipe` struct is the envelope shape); the existing `crates/autotrain/tests/fixtures/testnet-live-proof-fixture/` (the no-receipt fallback is hard-coded to this path); the receipt directory layout `scripts/testnet-live-proof.sh` writes (the route reads what the runbook wrote — same shape, no schema change). **Estimated scope:** S (one new router file + one new state file + one new test file + 1 static-html edit + 1 deploy-doc note + 1 plan-row update = ~250 lines of new code/tests). **Completion signal:** a fresh `curl http://localhost:8080/api/receipt/latest` after STW-095 + STW-096 land returns 200 with the green receipt from STW-095 (the v10 → v11 adapter is live; a stranger can audit the testnet claim without `git clone`); the next planner pass can promote a follow-on slice (e.g. `wrangler r2 object put` the receipt on deploy, or extend the route to serve multiple receipts as a sorted list). **`lens:` CEO (the v10 dashboard ships the bench-card aggregator; the v10 → v11 adapter is the receipt the runbook commits — without it, a stranger can `curl /api/index` and see the bench cards but cannot `curl` the receipt that produced them; the v10 → v11 adapter closes the gap the README's `## Public dashboard` section implies but the v10 router did not surface) + Eng (the `serve_receipt` handler is a 100-line file-read + serde_json wrapper, no tokio-postgres, no new dep; the existing `IndexClient` + typed-error pattern is the model; a worker can implement the slice in 1-2 days) + Design (the dashboard's empty-state gains a one-line link so a first-time visitor can audit the most-recent receipt without writing `curl + jq`; the v10 → v11 follow-on is *additive* — the v10 surface is unchanged, the v11 surface is the new typed-read of the receipt).**

- [ ] **[P1] `STW-097` `OPERATOR-DECISIONS` (NEW in RE-PLAN-009; absorbs STW-074 from RE-PLAN-002 / RE-PLAN-003 / RE-PLAN-004 / RE-PLAN-005 / RE-PLAN-006 / RE-PLAN-007 / RE-PLAN-008) — close the 2 `[!]` operator-decision rows (`STW-001` planning surface, `STW-007` artifact retirement) with a recorded decision the next planner pass can promote against. **`STW-001`** resolves to "hand-author a queue in `IMPLEMENTATION_PLAN.md` (the current plan IS the queue; `gbrain` is not required for the next 3 months of work — `genesis/plans/000-ceo-testnet-roadmap.md` is evidence, `IMPLEMENTATION_PLAN.md` is the queue)" or "block on gbrain init". **`STW-007`** resolves to a per-path retention verdict for `.gbrain-source` (delete or keep), `.auto/tui*/` (delete or keep), `.auto/orchestrator/velocity-*` (delete), `.auto/corpus-staging/` (delete), `.auto/logs/steward-*-prompt.md` (delete), with the verdict recorded in a new `steward/ARTIFACT-RETENTION.md` file the planner can grep. **STW-097 is the only row in RE-PLAN-009 that genuinely blocks on human input** (the `.auto/` + `.gbrain-source` retention verdicts); every other row is a code/markdown change a worker can ship solo. The 6-RE-PLAN-cycle un-claim history STW-074 accumulated (RE-PLAN-002 → RE-PLAN-003 → RE-PLAN-004 → RE-PLAN-005 → RE-PLAN-006 → RE-PLAN-007 → RE-PLAN-008 — 7 cycles un-claimed) is the planner's signal that this row needs the operator's input, not a worker. **Owner files:** `IMPLEMENTATION_PLAN.md` (the `## Deferred items (need operator decision before promotion)` section is rewritten to a `## Operator decisions (RESOLVED 2026-06-10 by STW-097)` section with the verdict for each row + a one-paragraph rationale; the new `STW-097` row flips to `[x]` once recorded), `steward/ARTIFACT-RETENTION.md` (new — a per-path verdict table: `path | verdict | rationale | signoff`, with the `STW-097` row's resolution as the contents), `steward/HAZARDS.md` (the `STW-001` + `STW-007` rows flip from "open" to "closed" with a one-line `closed by STW-097 on <date>` note), `steward/PROMOTIONS.md` (the `STW-001` / `STW-007` `deferred` row is replaced with a `STW-097` `promoted` row), `steward/DRIFT.md` (the `STW-001` + `STW-007` rows in the `Blocked / Deferred` table are updated from `DRIFT` / `AGREES` to `RESOLVED` with the verdict). **Scope boundary:** do NOT execute the deletion (the planner is the recording desk, not the cleanup crew — a separate operator-side slice deletes after sign-off); do NOT change the `.gitignore`; do NOT remove `genesis/plans/000-ceo-testnet-roadmap.md` (it is the historical record the planner audits against); do NOT change runtime code; do NOT touch the 13 ghost rows `STW-071` retires; do NOT touch the RE-PLAN-009 STW-094/095/096 work. **Acceptance criteria:** the two `[!]` rows in `IMPLEMENTATION_PLAN.md` are replaced with `[x] STW-NNN RESOLVED 2026-06-10 by STW-097` rows + a one-paragraph verdict each; `steward/ARTIFACT-RETENTION.md` is a clean per-path verdict table; `steward/{HAZARDS,PROMOTIONS,DRIFT}.md` are updated in lockstep; a `rg -n "^- \[!\]" IMPLEMENTATION_PLAN.md` returns no rows. **Verification commands:** `rg -n "^- \[!\]" IMPLEMENTATION_PLAN.md` (should be empty), `rg -n "STW-001|STW-007" IMPLEMENTATION_PLAN.md steward/` (should show only `RESOLVED ... by STW-097` references), `bash scripts/plan-staleness-gate.sh` (must stay green — no new ghosts introduced), `cargo test --workspace -- --test-threads=4` (no code change so this is a regression check, not a feature check), `cargo check --workspace`, `cargo fmt --check`. **Hand-test:** a planner scanning `IMPLEMENTATION_PLAN.md` no longer sees 2 `[!]` rows; the next `auto steward --report-only` pass has a clean `Blocked / Deferred` table to start from. **Dependencies:** operator input on the 6 `.auto/` + `.gbrain-source` retention verdicts (this is the only row in RE-PLAN-009 that genuinely blocks on human input — every other row is a code/markdown change a worker can ship solo). **Estimated scope:** S (markdown only, no code). **Completion signal:** `rg -n "^- \[!\]" IMPLEMENTATION_PLAN.md` returns no rows; the next `auto steward --report-only` pass promotes against a clean decision table, not 2 deferred items + 13 ghost rows + 0 real next slices; the operator-side deletion slice (out of scope for STW-097) can pick up the per-path verdicts from `steward/ARTIFACT-RETENTION.md` and execute the deletion. **`lens:` CEO (the deferred items are the noise floor of every planner pass — recording a verdict turns the noise into a signal; the 7-cycle un-claim history is the planner's signal that this row needs the operator's input, not a worker) + Eng (markdown-only change; no code risk; the 6 `.auto/` retention verdicts are the operator's call, not a code change) + Design (a planner sees a decision table, not a `[!]` warning; the `steward/ARTIFACT-RETENTION.md` is the durable record a future planner can grep).**


---

## Next-phase active items (RE-PLAN-010 2026-06-11 by designcritic, RE-PLAN task t_56707c3b; supersedes RE-PLAN-009 2026-06-10 by designcritic, RE-PLAN task t_e00cbf38)

Audit at 01:36 UTC, 2026-06-11 (post-commit-7bfde14 + post-staged-STW-098-working-tree):

- **RE-PLAN-009 is not exhausted; it is *partially landed*.** The 4 rows RE-PLAN-009 added (STW-094 P0 + STW-095 P0 + STW-096 P1 + STW-097 P1) are real, but STW-094 is `[ ]` in the plan and `[x]` on `main` (commit 7bfde14 shipped the integrated STW-091 lookup-cap + STW-088 property-test layer on 2026-06-10 21:37:12Z) — the plan-vs-reality drift RE-PLAN-008 named "stale working tree" and RE-PLAN-009 named "the STW-094 integrated commit" is *closed on `main`* but the plan is *not flipped*. STW-095 cannot run on a fresh DB today because the fast-mode preflop build panics on the partial fast-mode flop lookup (the receipts `receipts/testnet-live-proof-20260611T000206Z_v3/` + `receipts/testnet-live-proof-20260611T000534Z_v3/` prove it: `cluster/exit.txt = 101`, `cluster/stdout.txt` ends at `kmeans hydrating preflop`, the panic is at `lookup.rs:43` with `precomputed abstraction in lookup`). The fix is structurally complete in the working tree (the STW-098 staged code in 5 files: `.gitignore` + `crates/clustering/src/layer.rs` + `crates/clustering/src/tests.rs` + `scripts/testnet-live-proof.{sh,md}` — 258 insertions, all gated on `RBP_TESTNET_FAST=1`, production byte-identical when the switch is unset) but uncommitted. STW-098 is the slice that commits it. RE-PLAN-010 names STW-098, names a re-issued STW-099 that runs the green receipt post-STW-098, names a server-public-surface hardening slice the receipts reveal as the next-largest gap, and names a queue-honesty cleanup.

- **The "current roadmap" the RE-PLAN-010 task body names is *RE-PLAN-009*, not "exhausted".** RE-PLAN-009 was authored 7 hours ago at RE-PLAN task t_e00cbf38 and shipped 4 new rows + 4 ghost-flip retirements; the body of the RE-PLAN-010 task is from a planning surface that pre-dates RE-PLAN-009. RE-PLAN-010's job is *not* to re-issue RE-PLAN-009; it is to *complete* the cycle RE-PLAN-009 opened (the integrated STW-094 commit landed; the testnet evidence slice STW-095 needs STW-098 to succeed) and to *pivot* to the next north star the receipt chain reveals (the server-public-surface gap, not the trainer-receipt chain).

- **The post-7bfde14 receipts reveal a 4th audit fact the RE-PLAN-009 author did not see:** the receipts dir is no longer a 33-failed-receipt graveyard — it is a 32-failed-archived + 1-un-archived (`testnet-live-proof-20260604T052134Z`) + 2-fresh-failed-`_v3` graveyard. The RE-PLAN-009 STW-095 spec described a "move 33 failed receipts" + "write INDEX.md" + "add `!receipts/_archive/`" but the on-disk state is "32 moved, 1 not moved, 2 fresh failures, no INDEX.md, the `.gitignore` is staged but uncommitted, the runbook `_v3` suffix is staged but uncommitted." STW-099 in RE-PLAN-010 re-issues the archive work with the truthful on-disk count (32 + 1 + 2 = 35 total archived-failed once the un-archived one + the 2 fresh `_v3` failures are moved; the INDEX.md is a real audit trail of 35 bullets, not 33) and the staged runbook + .gitignore work is the spec for STW-098's commit.

What RE-PLAN-010 changes versus RE-PLAN-009:

- **STW-098 (NEW, P0) — `PREFLOP-FAST-MODE-HYDRATION-FIX`.** The 4th audit finding the RE-PLAN-009 author did not see: even with STW-094's lookup cap, the fast-mode preflop build path panics on the partial fast-mode flop lookup (`Lookup::projections()` -> `future()` -> `lookup(child)` panics on a missing child iso). The preflop iso iterator enumerates all preflop isos, so most `lookup(child)` calls miss on the partial fast-mode flop lookup. The fix is structurally complete in the working tree (5 files, 258 insertions, all gated on `RBP_TESTNET_FAST=1` so production is byte-identical). The slice is the *integrated* commit the worker lands: review the staged code, run `cargo test -p rbp-clustering`, run `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` against a fresh DB, confirm the `cluster=0` exit (not `101`), commit. See the full STW-098 row below for owner files, scope boundary, acceptance criteria.

- **STW-099 (NEW, P0) — `TESTNET-LIVE-PROOF-RECEIPT-POST-STW-098` (RE-PLAN-010 re-issue of STW-095 from RE-PLAN-009 with the corrected dependency chain).** Run `scripts/testnet-live-proof.sh` end-to-end with `RBP_TESTNET_FAST=1` against a fresh STW-078-provisioned Postgres, with `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` (commit 7bfde14) + `STW-098` all shipped, and commit the first green `receipts/testnet-live-proof-<UTC-THIS-WEEK>_v3/` directory the receipt chain has produced. STW-099 also executes the STW-085 archive work RE-PLAN-005 described (RE-PLAN-009 + RE-PLAN-010 promote STW-085 from P1 to P0-dependency of STW-099): move the 32 already-archived receipts + the 1 un-archived pre-archive + the 2 fresh `_v3` failures (35 total) into `receipts/_archive/failed/` (the staged runbook + .gitignore work covers the path conventions) + write a real `receipts/_archive/failed/INDEX.md` (35 bullets: 32 from RE-PLAN-009's spec + the 1 un-archived `20260604T052134Z` + the 2 fresh `_v3` failures `20260611T000206Z_v3` + `20260611T000534Z_v3`, each with the failure mode + the slice that fixed it: `20260611T000534Z_v3: cluster=panic at lookup.rs:43 precomputed_abstraction_in_lookup during preflop kmeans hydration, fixed by STW-098 synthetic fast-flop lookup`) + close the .gitignore + runbook-doc + INDEX.md integration that RE-PLAN-009's STW-095 spec described. The post-STW-098 runbook run reaches `--bench` + `--compare` + `--replay` in well under 5 minutes per street (the dominant per-street cost is now the 1024-row lookup, not the 1.3M-row lookup, AND the preflop hydration no longer panics on the partial flop lookup).

- **STW-100 (NEW, P0) — `SERVER-CI-TEST-GATE` (RE-PLAN-010 re-issue of STW-080 + STW-081 from RE-PLAN-005; the 2-cycle un-claim history of STW-080/081 is the planner's signal that this slice needs a *bigger* surface, not a smaller one).** Close the largest test-coverage gap in the product + the largest CI gap: `crates/server` has 24 routes (4 `/auth/*` + 4 `/room/*` + 15 `/api/*` + 1 `/health`, per `crates/server/src/lib.rs:59-105`) and ZERO HTTP-level integration tests (the 2 existing test files `crates/server/tests/analysis_cli.rs` + `crates/server/tests/dto_wire.rs` exercise only the no-DB no-actix `render_query` helper + the DTO wire format, not the routes). `.github/workflows/ci.yml` runs `cargo test --lib --quiet` only, so NONE of the 28 integration test files (`crates/{server,database,auth,autotrain,nlhe,dashboard,clustering,gameroom}/tests/*.rs`) run in CI. The `verification:workspace-parallel` hinge is *mechanically* closed (STW-020 + STW-030) but *not actually green in CI* — a future regression in any integration test is invisible to the dashboard badge. STW-100 is one integrated commit: (a) add `crates/server/tests/http_routes.rs` (no-DB `actix_web::test` integration tests that drive the 24 routes through `actix_web::test::init_service` + assert status codes + JSON envelope shapes — the existing `crates/server/tests/dto_wire.rs` is the DTO-shape model; the existing `crates/dashboard/tests/smoke.rs` is the axum-router model; STW-100 introduces the actix-router model); (b) extend `.github/workflows/ci.yml` to add a `cargo test --tests --quiet` step (mirroring the existing `cargo test --lib --quiet` step, in a separate job so integration test failures don't break the lib-test badge); (c) add `crates/autotrain/tests/script_shape.rs::ci_workflow_runs_integration_tests` pin (a `bash -n` + a `rg -n "cargo test --tests" .github/workflows/ci.yml` shape pin so a future refactor that drops the integration-test step fails the pin). The slice is large but bounded: ~24 route sub-tests (one per route, asserting status + envelope shape) + 1 CI step + 1 shape pin = ~600 lines of new code/tests + 1 workflow file edit.

- **STW-101 (NEW, P1) — `SERVER-DEPLOY-RUNBOOK + HTTP-API-DOCS` (RE-PLAN-010 re-issue of STW-082 + STW-083 from RE-PLAN-005).** Close the deploy + docs gap the post-STW-100 server tests reveal: after the 24 routes have green tests + the CI gate is green, a stranger still has no way to bring up the server or learn what the 24 routes do. STW-101 ships: (a) `scripts/deploy-server.sh` (the operator-runnable deploy runbook — `cargo build --release -p rbp-server` + a `RBP_SERVER_BIND_ADDR` + `RBP_SERVER_DATABASE_URL` env-knob block + a `bash -n` shape pin mirroring `scripts/deploy-dashboard-cloudflare.sh`); (b) `scripts/deploy-server.md` (the env-knob table + the pre-deploy DB migration check + the post-deploy `curl /health` smoke test); (c) a new `## HTTP API` section in `README.md` covering the 24 routes with one curl example per route + the expected status code + the JSON envelope shape (mirroring the existing `## Testnet launch proof` + `## Public dashboard` sections; the new section is 24 route rows + 24 curl examples = ~250 lines); (d) `crates/autotrain/tests/script_shape.rs` extends the `runbook_doc_lists_every_env_knob` pin to cover the 2 new server env knobs + the `readme_has_http_api_section` pin (a `rg -n "## HTTP API" README.md` pin).

- **STW-102 (NEW, P1) — `PLAN-CHECKBOX-HONESTY-AUDIT-POST-STW-098` (RE-PLAN-010 plan-hygiene slice).** The post-7bfde14 audit reveals 4 P0 ghosts in the plan: STW-091 + STW-092 + STW-093 + STW-094 are `[ ] [P0]` in the plan but `[x]` on `main` (commit 7bfde14 shipped the integrated work). The plan-staleness-gate does not catch them (the gate's P0 pass scans only the roadmap, the gate's P1 pass scans `[ ] [P1]` not `[ ] [P0]`). The 4 flips are XS (4 `- [ ] **` → `- [x] **(RETIRED 2026-06-11 by RE-PLAN-010; see evidence row below)**` prefix-only replacements) + a new `scripts/plan-staleness-gate.sh` P0 pass that scans `[ ] [P0]` rows in the plan (mirroring the existing `[ ] [P1]` P1 pass; the new pass is ~15 lines + 1 sub-test in `crates/autotrain/tests/plan_staleness.rs`). After STW-102 lands, the active `[ ] [P0]` queue in the plan is 2 rows (STW-098 + STW-099, the post-STW-094 work) + the 9 carry-over P1s + the 4 new RE-PLAN-010 rows.

- **The 11 carry-over `[ ]` rows from RE-PLAN-004/005/009** (STW-071 P0 + STW-079 P1 + STW-080 P0 [re-issued as STW-100] + STW-081 P0 [re-issued as STW-100] + STW-082 P0 [re-issued as STW-101] + STW-083 P1 [re-issued as STW-101] + STW-084 P1 + STW-085 P0-dep of STW-099 [was STW-095] + STW-095 P0 [re-issued as STW-099] + STW-096 P1 + STW-097 P1) **carry over unchanged.** The 4 P0 ghosts RE-PLAN-010 retires (STW-091 + STW-092 + STW-093 + STW-094) are flipped in STW-102, not in-place here.

- **No new P0 rows beyond STW-098 + STW-099 + STW-100.** The 5 P0 dependencies STW-099 names are all shipped (`STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087`); the 2 new dependencies STW-099 names (STW-094 commit 7bfde14 + STW-098) are the 2 RE-PLAN-010 P0 slices. STW-100 is the next-north-star P0 (server-public-surface hardening, the gap the green-receipt closure reveals).

The 5 new rows below (STW-098 P0 + STW-099 P0 + STW-100 P0 + STW-101 P1 + STW-102 P1) are the minimum surface that closes the planning cycle from "RE-PLAN-009 leaves 4 P0 ghosts in the plan + 2 fresh `_v3` failures in receipts + the testnet-evidence slice blocked on a preflop hydration panic + the server-public-surface gap un-named" to "RE-PLAN-010 is green: 1 commit ships the preflop hydration fix + 1 runbook run lands the green receipt + 1 commit ships the server HTTP integration tests + 1 commit ships the deploy runbook + API docs + 1 markdown-only slice flips the 4 P0 ghosts. Total post-RE-PLAN-010 active open `[ ]` rows: 11 (4 RE-PLAN-004/005/009 P0/P1 + 1 STW-084 + 1 STW-085 + 1 STW-096 + 1 STW-097 + 3 RE-PLAN-010 P0 + 1 RE-PLAN-010 P1; STW-080/081/082/083/095 are folded into STW-100/100/101/101/099). A `rg -n "^- \[ \] \*\*\[P[01]\]" IMPLEMENTATION_PLAN.md` after this commit shows STW-071, STW-079 (RE-PLAN-004) + STW-084, STW-085, STW-096, STW-097 (RE-PLAN-005/009) + STW-098, STW-099, STW-100, STW-101, STW-102 (RE-PLAN-010 NEW). Priority order: **STW-098 is the smallest P0 with the highest leverage** (it is the staged working tree the receipts prove is needed; 1 commit closes the working tree + unblocks STW-099); **STW-099 is the terminal evidence slice** (the green receipt the testnet north star demands); **STW-100 is the next-north-star P0** (server-public-surface hardening, the gap a green-receipt closure reveals); **STW-101 is the deploy + docs follow-on** (the 24-route surface STW-100 tests needs to be documented + deployable); **STW-102 is the plan-hygiene slice** (4 P0 ghost flips + 1 gate extension). Owner for the RE-PLAN row itself: designcritic, 2026-06-11.

The 5 new rows below mirror this priority order.

- [ ] **[P0] `STW-098` `PREFLOP-FAST-MODE-HYDRATION-FIX` (NEW in RE-PLAN-010; the fix is structurally complete in the working tree at 01:36 UTC 2026-06-11, 5 files staged with 258 insertions, all gated on `RBP_TESTNET_FAST=1` so production is byte-identical when the switch is unset).** The 2026-06-11 00:05:34Z testnet-live-proof runbook run captured a 4th receipt-blocking failure mode the RE-PLAN-009 author did not see: the fast-mode preflop build path panics at `lookup.rs:43` with `precomputed abstraction in lookup` mid-preflop-kmeans-hydration. The preflop iso iterator enumerates all preflop isos, so most `lookup(child)` calls miss on the partial fast-mode flop lookup (STW-094's `Layer::lookup_with_prefix` cap is a 1024-row prefix, but the preflop kmeans driver needs a *complete* flop lookup to compute `projections()`). The fix is mechanically simple: in the fast-mode preflop build path, synthesize a complete (degenerate) in-memory flop lookup — one entry per flop iso, each mapping to `Abstraction::from((Street::Flop, 0))`. The synthesized lookup is degenerate (every flop iso lands in bucket 0) but it is complete (every preflop iso's child is present), so the preflop kmeans driver sees a well-formed (degenerate) point pool instead of panicking on a missing key. The fast-mode preflop clustering result is meaningless for production use (fast mode is a smoke-test path, not a training path; the preflop cluster map is never used downstream) but the testnet-live-proof runbook now exits `0` instead of `101`. **The fix is in the working tree** (5 files: `.gitignore` (the `!receipts/_archive/` exception the STW-099 spec needs) + `crates/clustering/src/layer.rs` (the `synthetic_fast_flop_lookup()` helper + the `next_lookup` ternary in `Layer::build`'s preflop arm) + `crates/clustering/src/tests.rs` (the `synthetic_fast_flop_lookup_is_complete_and_degenerate` regression test) + `scripts/testnet-live-proof.sh` (the `_v2` -> `_v3` suffix bump + the `RBP_FAST_LOOKUP_SAMPLE` knob wiring) + `scripts/testnet-live-proof.md` (the new `## Receipts archive (receipts/_archive/failed/) — STW-099` section + the env-knob table row). The worker reads the staged code, validates it (`cargo test -p rbp-clustering --lib` + `cargo test -p rbp-clustering --tests` + a `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` against a fresh STW-078-provisioned Postgres that exits 0 not 101), and commits. **Two acceptance paths, both individually shippable, both end with `git status` clean:** (path-a, "ship the staged code as-is") — the worker reads the 5 staged files + judges the staged code matches the RE-PLAN-010 spec + commits as a single `feat(clustering): STW-098 preflop fast-mode synthetic flop lookup + runbook v3 + archive exception` commit. The commit body names `STW-098` as the integrated slice + the 2 fresh `_v3` failures the fix closes + the 4 P0-ghost flips STW-102 will name. **(Path-b, "revert the staged state + re-issue as XS")** — the worker reads the staged code + judges it NOT shippable (e.g. the synthetic-lookup is too degenerate for a future refactor, the test pin is too tight, the runbook `_v3` suffix collides with a future plan) + reverts the 5 staged files (`git checkout HEAD -- .gitignore crates/clustering/src/layer.rs crates/clustering/src/tests.rs scripts/testnet-live-proof.sh scripts/testnet-live-proof.md`) + re-issues `STW-098` in RE-PLAN-011 with the corrected spec + an audit-trail note in `steward/WORKING-TREE-DECISIONS.md`. **The path-a / path-b decision is the worker's** (RE-PLAN-010 intentionally does not make the choice). **Acceptance criteria for the integrated commit:** `git status` is clean post-commit; `cargo test -p rbp-clustering --lib` is green (no regression in the 18 lib sub-tests); `cargo test -p rbp-clustering --tests` is green (no regression in the 5 `kmeans_fast` + 5 `kmeans_property` + 3 `lookup_fast` sub-tests; the new `synthetic_fast_flop_lookup_is_complete_and_degenerate` regression test is green); `cargo test -p rbp-autotrain --test script_shape` is green (the `runbook_doc_lists_every_env_knob` pin is extended to cover `RBP_FAST_LOOKUP_SAMPLE`); `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with `plan staleness gate complete: checked=N ghosts=0` (the staged .gitignore + runbook + doc changes are STW-098's spec, not the gate's concern; the gate scans the plan, not the runbook). **Hand-test:** a fresh `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` against a fresh STW-078-provisioned Postgres reaches `--bench` in well under 5 minutes per street (the dominant per-street cost is now the 1024-row lookup + the degenerate-preflop hydration, not the 1.3M-row lookup + the panic); the receipt's `cluster/exit.txt` is `0`, not `101`; the receipt's `cluster/stdout.txt` shows the `kmeans hydrating preflop` step completing (not hanging on the `lookup.rs:43` panic) + a new `synthetic fast-mode flop lookup: 1286792 entries` log line. **Scope boundary:** do NOT change the production `Layer::build` path (the synthetic-lookup branch is gated on `rbp_core::testnet_fast() && street == Street::Pref`; the production preflop build is byte-identical when `RBP_TESTNET_FAST` is unset); do NOT change the `Layer::lookup` STW-091 cap (the cap is unchanged; STW-098 is on the preflop BUILD path, not the lookup-construction path); do NOT change the kmeans driver (the `run_fast` / `init_kmeans_plus_plus` paths are unchanged; the degenerate-preflop point pool is a pre-existing-panic fix, not a kmeans change); do NOT change the `RBP_FAST_LOOKUP_SAMPLE` default (1024, set by STW-094; STW-098 honors the default); do NOT change the runbook's chain step order; do NOT change the `LiveProofReceipt::read_and_verify` verifier; do NOT touch any v1/v2/v3 profile schema, the dashboard, the publish/index/remote chain, the seat-aware work, the autotrain pipeline, or any `trainer --*` CLI; do NOT touch the v6 -> v10 follow-on chain narrative earlier in the plan. **Dependencies:** the working tree's STW-098 code (already drafted — the worker ports + decides + commits); the existing `crates/clustering/src/layer.rs` (the preflop-build arm STW-098 modifies); the existing `crates/clustering/src/lookup.rs` (the `Lookup::projections` / `Lookup::future` / `lookup` methods STW-098's synthetic-lookup must satisfy). **Estimated scope:** S (one commit + ~30 lines of new code in `layer.rs` + ~30 lines of new tests in `tests.rs` + ~20 lines of new runbook + doc + .gitignore wiring = 258 insertions total per the staged diff). **Completion signal:** `git log -1 --oneline` on `main` shows a new commit titled `feat(clustering): STW-098 preflop fast-mode synthetic flop lookup + runbook v3 + archive exception`; `git status` is clean post-commit; a fresh `RBP_TESTNET_FAST=1 bash scripts/testnet-live-proof.sh` no longer panics at `lookup.rs:43` (the receipt's `cluster/stdout.txt` shows the `kmeans hydrating preflop` step completing + the `synthetic fast-mode flop lookup: 1286792 entries` log line + a `cluster=0` exit); `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts. **`lens:` CEO (the v1->v10 testnet chain was *structurally* complete post-STW-094; STW-098 closes the 4th receipt-blocking failure mode the post-STW-094 receipts revealed — the receipts are the planner's signal, not a wish-list) + Eng (the synthetic-lookup helper is 30 lines; the regression test is 30 lines; the runbook + doc + .gitignore wiring is 20 lines; the integration commit is bounded and verifiable) + Design (the receipts dir a future planner opens shows the *honest* state: the `_v3` failures are diagnosed and the fix is integrated, not 33+2 confusing stale dirs + 0 green ones + a half-drafted working tree).**

- [ ] **[P0] `STW-099` `TESTNET-LIVE-PROOF-RECEIPT-POST-STW-098` (NEW in RE-PLAN-010; absorbs STW-095 from RE-PLAN-009 + the STW-085 archive work from RE-PLAN-005, with the truthful post-STW-098 dependency chain).** Run `scripts/testnet-live-proof.sh` end-to-end with `RBP_TESTNET_FAST=1` against a fresh STW-078-provisioned Postgres, with `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` (commit 7bfde14) + `STW-098` all shipped, and commit the first green `receipts/testnet-live-proof-<UTC-THIS-WEEK>_v3/` directory the receipt chain has produced. **The truthful dependency chain is `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` (commit 7bfde14) + `STW-098`**, and STW-099 is the slice that runs the runbook end-to-end post-STW-098. This is the *terminal evidence* slice: it converts the v1->v10 testnet chain from "structurally complete, kmeans-panic-blocked AND lookup-hang-blocked AND preflop-hydration-panic-blocked" to "operationally proven." The runbook's 8 step exits (`doctor` / `cluster` / `reset` / `smoke` / `status` / `bench` / `compare` / `replay`) must all be `0`; the `SUMMARY.txt` headline must be the pinned `testnet live_proof complete: smoke=N status=N bench=N compare=N replay=BYTES` line; the `recipe.json` must re-verify with `LiveProofReceipt::read_and_verify`; the receipt is committed on `main`. STW-099 is *evidence only*: the runbook is run, the receipt is captured, the verifier is invoked, the result is committed. If a fresh runbook run fails after STW-098 lands, the worker reports the failure as a new `[P0]`-ranked next slice (or blocks for human input) rather than silently fixing the runbook in the STW-099 commit. **STW-099 also executes the STW-085 archive work RE-PLAN-005 described (RE-PLAN-009 + RE-PLAN-010 promote STW-085 from P1 to P0-dependency of STW-099 because the receipts dir is currently a graveyard):** move the 35 failed/partial `receipts/testnet-live-proof-*` directories into `receipts/_archive/failed/` (32 already in the archive from prior work + 1 un-archived `20260604T052134Z` + 2 fresh `_v3` failures `20260611T000206Z_v3` + `20260611T000534Z_v3` = 35 total) + write a real `receipts/_archive/failed/INDEX.md` (committed, ~50 lines; one bullet per archived receipt + the failure mode + the slice that fixed it, in chronological order: `20260604T140107Z..20260604T164243Z (7 receipts): cluster=panic + runbook-shape drift, fixed by STW-075 + STW-019 + STW-022`, ..., `20260609T042107Z: cluster=hang at kmeans iterating on turn, fixed by STW-077 fast-mode sample/iteration cap`, ..., `20260610T112026Z: cluster=hang at calculating_lookup_flop, fixed by STW-094 lookup fast-mode cap`, ..., `20260611T000206Z_v3: cluster=panic at lookup.rs:43 preflop kmeans hydration, fixed by STW-098 synthetic fast-flop lookup`, `20260611T000534Z_v3: cluster=panic at lookup.rs:43 preflop kmeans hydration, fixed by STW-098 synthetic fast-flop lookup`). The `.gitignore` `!receipts/_archive/` exception STW-098's runbook-update adds is the path the committed `INDEX.md` + the 35 archived subdirs travel on. The `scripts/testnet-live-proof.sh` `_v3` suffix STW-098's runbook-update adds is the path the *new* green receipt lands on. **Owner files:** `scripts/testnet-live-proof.sh` (no code change expected beyond what STW-098's runbook-update already ships; the `RBP_FAST_LOOKUP_SAMPLE` knob is set by STW-098's runbook-update), `scripts/testnet-live-proof.md` (the new `## Receipts archive (receipts/_archive/failed/) — STW-099` section STW-098's doc-update already ships), the new green receipt directory under `receipts/testnet-live-proof-<UTC>_v3/` (the deliverable), `receipts/_archive/failed/INDEX.md` (the 35-bullet failure-history summary, ~50 lines), `.gitignore` (the `!receipts/_archive/` exception STW-098's runbook-update already ships), `steward/HAZARDS.md` (the `TESTNET-LIVE-PROOF-RECEIPT` row flips from "open" to "closed by STW-099 on <date>" once the receipt is committed), `steward/DRIFT.md` (the `STW-019` row's "DRIFT" verdict on the receipts-orphans line flips to "RESOLVED by STW-099 on <date>" once the new receipt is committed; the `STW-085` row flips from "open" to "closed by STW-099 on <date>" once the archive + INDEX are committed). **Scope boundary:** do NOT change the runbook's chain step order; do NOT weaken `LiveProofReceipt::read_and_verify`; do NOT bypass `trainer --doctor`; do NOT touch the `Check::clustered` fix (that is `STW-075`'s commit, not this one); do NOT touch the kmeans cap (that is `STW-077`'s commit, not this one); do NOT touch the postgres provisioning script (that is `STW-078`'s commit, not this one); do NOT touch the kmeans empty-histogram pre-filter (that is `STW-086`'s commit, not this one); do NOT touch the kmeans empty-cluster guard (that is `STW-087`'s commit, not this one); do NOT touch the kmeans lookup fast-mode cap (that is `STW-094`'s commit, not this one); do NOT touch the preflop hydration fix (that is `STW-098`'s commit, not this one); do NOT touch the working-tree discipline (that is `STW-102`'s commit, not this one); do NOT touch the dashboard / publish / index / index-remote / remote chain; do NOT delete the existing failed/partial `receipts/testnet-live-proof-*` directories (they are archived, not deleted); do NOT change the `RBP_FAST_LOOKUP_SAMPLE` default (1024, set by STW-094; STW-098 honors the default; STW-099 honors the default). **Acceptance criteria:** a `find receipts/testnet-live-proof-<UTC>_v3/` shows 8 step subdirs each with `stdout.txt` + `stderr.txt` + `exit.txt` where every `exit.txt` is `0`; `SUMMARY.txt` head is the pinned `testnet live_proof complete: ...` line; `recipe.json` parses with `LiveProofRecipe`; `trainer --verify-receipt receipts/testnet-live-proof-<UTC>_v3/` exits 0 and prints `live_proof receipt verification passed: ...`; the receipt is committed on `main`; the `receipts/_archive/failed/` directory contains all 35 archived receipts + the `INDEX.md` summary; `git status` is clean post-commit; `steward/HAZARDS.md` + `steward/DRIFT.md` + `steward/PROMOTIONS.md` reflect the closed `TESTNET-LIVE-PROOF-RECEIPT` hinge + the closed `STW-085` archive row. **Dependencies (RE-PLAN-010):** `STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` (commit 7bfde14) + `STW-098` + a reachable `DATABASE_URL` or `DB_URL`; existing `STW-067` (fast-mode) + `STW-069` (fail-before-train integrity gate) + `STW-019` (runbook) + `STW-023` (shared verifier + `recipe.json`) + `STW-028` (`--verify-receipt` CLI) + `STW-085` (the FAILED-RECEIPT-ARCHIVE spec STW-099 integrates + promotes from RE-PLAN-005 P1 to STW-099 P0-dependency). **Estimated scope:** S (operator-runnable evidence, on a fresh DB, post-`STW-075` + `STW-077` + `STW-078` + `STW-086` + `STW-087` + `STW-094` + `STW-098`; the receipt capture + the archive move of the 35 receipts + the 35-bullet INDEX.md write + the steward updates = 1 integrated commit, not 4). **Completion signal:** a fresh `trainer --verify-receipt` exits 0 on a `receipts/testnet-live-proof-<UTC-THIS-WEEK>_v3/` directory and the receipt path is recorded in `steward/HAZARDS.md` row #2 + `steward/DRIFT.md` `STW-019` row as the current operator proof; the receipts dir shows only the new green receipt + the `_archive/failed/` summary; the `verification:workspace-parallel` hinge #2 is now `closed by STW-099 on <date>` with the receipt path as evidence; the next planner pass sees a `RE-PLAN-011` task with no `STW-091` / `STW-092` / `STW-093` / `STW-094` / `STW-095` / `STW-098` / `STW-099` carry-over (the receipt chain is *evidenced*; the planner can move to a different north star — the v10 -> v11 dashboard/receipt adapter (STW-096, the next slice after STW-099), the server-public-surface hardening (STW-100, the next-north-star P0), the operator-decision closeout (STW-097), or the seat-aware slice 5 export (STW-073, deferred)). **`lens:` CEO (the testnet claim requires a green receipt; STW-099 is the slice that converts "structurally complete" to "operationally proven" with the *correct* dependency chain — the 7 RE-PLAN cycles of STW-070 re-issues have all been correct in spirit (the chain is structurally complete) but wrong in detail (each re-issue missed a layer: the kmeans cap was wrong for 2026-06-10 11:20, the lookup cap was right for STW-094, the preflop hydration fix is right for STW-098, STW-099 runs the receipt) + Eng (the runbook is run, the receipt is captured, the verifier is invoked, the 35 failed receipts are archived, the INDEX.md is written, the steward is updated — a single integrated commit, the same shape the STW-085 spec described, now elevated to P0 because the receipts dir is currently a graveyard) + Design (the receipts dir a stranger `ls`-es shows the *honest* state: one green `_v3` receipt + one archived failure-history summary with 35 bullets + the `_v3` suffix, not 35 confusing stale dirs + 0 green ones).**

- [ ] **[P0] `STW-100` `SERVER-CI-TEST-GATE` (NEW in RE-PLAN-010; absorbs STW-080 + STW-081 from RE-PLAN-005, with the truthful post-testnet-evidence scope).** Close the largest test-coverage gap in the product + the largest CI gap: `crates/server` has 24 routes (4 `/auth/*` + 4 `/room/*` + 15 `/api/*` + 1 `/health`, per `crates/server/src/lib.rs:59-105`) and ZERO HTTP-level integration tests (the 2 existing test files `crates/server/tests/analysis_cli.rs` + `crates/server/tests/dto_wire.rs` exercise only the no-DB no-actix `render_query` helper + the DTO wire format, not the routes). `.github/workflows/ci.yml` runs `cargo test --lib --quiet` only, so NONE of the 28 integration test files (`crates/{server,database,auth,autotrain,nlhe,dashboard,clustering,gameroom}/tests/*.rs`) run in CI. The `verification:workspace-parallel` hinge is *mechanically* closed (STW-020 + STW-030) but *not actually green in CI* — a future regression in any integration test is invisible to the dashboard badge. **STW-100 is one integrated commit:** (a) add `crates/server/tests/http_routes.rs` (a no-DB `actix_web::test` integration test file that drives the 24 routes through `actix_web::test::init_service` + asserts status codes + JSON envelope shapes — the existing `crates/server/tests/dto_wire.rs` is the DTO-shape model; the existing `crates/dashboard/tests/smoke.rs` is the axum-router model; STW-100 introduces the actix-router model for `crates/server`); the 24 route sub-tests are organized into 4 sections (auth: 4 routes, room: 4 routes, analysis/api: 15 routes, health: 1 route) with one `#[actix_web::test]` per route + one `assert_eq!(status, 200/4xx)` + one `assert!(envelope["field"] == expected)` shape assertion; routes that require a DB connection (e.g. `/api/blueprint` which queries the v1/v2/v3 profile tables) are stubbed with a `mock_db` helper that returns a typed error envelope (the existing `crates/autotrain/tests/script_shape.rs` mock-DB pattern is the model; the integration test asserts the error envelope, not the success envelope, for DB-bound routes so the test is no-DB but still exercises the route handler + the typed-error mapper); (b) extend `.github/workflows/ci.yml` to add a `cargo test --tests --quiet` step in a separate `integration-tests` job (mirroring the existing `cargo test --lib --quiet` step in the `unit-tests` job; the integration test job runs after the unit-test job so a future lib-test regression fails fast); (c) add `crates/autotrain/tests/script_shape.rs::ci_workflow_runs_integration_tests` pin (a `bash -n` shape pin + a `rg -n "cargo test --tests" .github/workflows/ci.yml` shape pin so a future refactor that drops the integration-test step fails the pin). **Acceptance criteria:** `cargo test -p rbp-server --test http_routes` is green with 24 sub-tests (one per route); `cargo test -p rbp-server --lib` stays green; `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts; `.github/workflows/ci.yml` on `main` has a `cargo test --tests --quiet` step in the `integration-tests` job; the GitHub Actions `integration-tests` check on the PR is green. **Scope boundary:** do NOT change the 24 route handlers (the test asserts the existing handler behavior, not a new behavior); do NOT change the actix-web dependency; do NOT add a new test database (the no-DB mock-DB helper is the pattern; the existing `crates/autotrain/tests/script_shape.rs` mock-DB is the model); do NOT change the `.github/workflows/ci.yml` `unit-tests` job (STW-100 adds a new `integration-tests` job, it does not modify the existing one); do NOT change the dashboard's `axum` router (the server is `actix-web`, the dashboard is `axum`; the test patterns are parallel but the crates are independent); do NOT touch any v1/v2/v3 profile schema, the trainer, the receipt chain, the publish/index/remote chain, the seat-aware work, or the runbook. **Owner files:** `crates/server/tests/http_routes.rs` (new no-DB integration test file, ~600 lines, 24 `#[actix_web::test]` sub-tests organized into 4 sections), `.github/workflows/ci.yml` (add the `integration-tests` job + the `cargo test --tests --quiet` step + the `actions/cache` for the target dir to keep the integration test job under 5 minutes wall-clock), `crates/autotrain/tests/script_shape.rs` (extend the `runbook_doc_lists_every_env_knob` pin with `ci_workflow_runs_integration_tests` shape pin, ~5 lines), `IMPLEMENTATION_PLAN.md` (this row), `steward/HAZARDS.md` (the `STW-080` + `STW-081` rows flip from "open" to "closed by STW-100 on <date>"). **Dependencies:** the existing `crates/server/src/lib.rs` (the 24 route definitions + the actix-web `App` builder the tests target); the existing `crates/server/tests/dto_wire.rs` (the DTO-shape assertion model); the existing `crates/dashboard/tests/smoke.rs` (the axum-router test model — `crates/server` mirrors it for actix); the existing `crates/autotrain/tests/script_shape.rs` (the mock-DB + shape-pin pattern); the existing `.github/workflows/ci.yml` (the unit-tests job STW-100's integration-tests job is parallel to). **Estimated scope:** L (one new test file with 24 sub-tests + one new CI job + one shape-pin extension = ~600 lines of new tests + 1 workflow file edit + 1 shape-pin addition). **Completion signal:** a fresh `cargo test -p rbp-server --test http_routes` exits 0 with 24 sub-tests green; a fresh `bash .github/workflows/ci.yml` simulation (via `act` or a local `cargo test --tests -p rbp-server` run) exits 0; the GitHub Actions `integration-tests` check on the next PR is green. **`lens:` CEO (the testnet-evidence slice is *one* of the north stars; the *other* north star is "can a stranger `curl` the server and get a sensible response?" — STW-100 is the slice that converts "we have a server" to "we have a server with green HTTP tests + a green CI gate"; STW-080/081 from RE-PLAN-005 were the right *idea* but the wrong *scope* — the 2-cycle un-claim history is the planner's signal that the slice needed to be a *single integrated commit*, not 2 separate P0s) + Eng (the 24 sub-tests are organized by route; the actix-web test harness is the standard pattern; the integration-test job is a copy of the unit-test job with `cargo test --tests` instead of `cargo test --lib`; the shape pin is 5 lines) + Design (a stranger `curl`-ing the server after STW-100 lands can trust the response — the green CI badge is the public signal the server is *intentionally* shipped, not a 24-route surface with zero test coverage + zero CI gate).**

- [ ] **[P1] `STW-101` `SERVER-DEPLOY-RUNBOOK + HTTP-API-DOCS` (NEW in RE-PLAN-010; absorbs STW-082 + STW-083 from RE-PLAN-005, with the truthful post-STW-100 scope).** Close the deploy + docs gap the post-STW-100 server tests reveal: after the 24 routes have green tests + the CI gate is green, a stranger still has no way to bring up the server or learn what the 24 routes do. **STW-101 is one integrated commit:** (a) `scripts/deploy-server.sh` (the operator-runnable deploy runbook — `cargo build --release -p rbp-server` + a `RBP_SERVER_BIND_ADDR` (default `0.0.0.0:8080`) + `RBP_SERVER_DATABASE_URL` (default `postgres://localhost/robopoker`) + `RBP_SERVER_LOG_LEVEL` (default `info`) env-knob block at the top + a pre-deploy DB migration check that runs `cargo run -p rbp-database -- migrate` + a post-deploy `curl http://localhost:8080/health` smoke test that asserts `{"status":"ok"}` + a `set -euo pipefail` shell discipline + a `bash -n` shape pin mirroring `scripts/deploy-dashboard-cloudflare.sh`'s shape pin); (b) `scripts/deploy-server.md` (the env-knob table + the pre-deploy DB migration check + the post-deploy smoke test + a `## Operator workflow` section that walks through `cargo build --release` -> `bash scripts/deploy-server.sh` -> `curl /health` -> `curl /api/index`; ~80 lines); (c) a new `## HTTP API` section in `README.md` covering the 24 routes with one curl example per route + the expected status code + the JSON envelope shape (mirroring the existing `## Testnet launch proof` + `## Public dashboard` sections; the new section is 24 route rows + 24 curl examples = ~250 lines; the section is organized by route-prefix: `/auth/*` (4 routes) + `/room/*` (4 routes) + `/api/*` (15 routes) + `/health` (1 route)); (d) `crates/autotrain/tests/script_shape.rs` extends the `runbook_doc_lists_every_env_knob` pin to cover the 3 new server env knobs + adds a `readme_has_http_api_section` pin (a `rg -n "## HTTP API" README.md` pin so a future refactor that drops the section fails the pin). **Acceptance criteria:** `bash -n scripts/deploy-server.sh` exits 0; `cargo test -p rbp-autotrain --test script_shape` is green with the new `deploy_server_script_parses_with_bash_n` + `runbook_doc_lists_every_env_knob` (extended) + `readme_has_http_api_section` sub-tests; `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts; a fresh `bash scripts/deploy-server.sh` on a local Postgres + a fresh `cargo run -p rbp-server` + a `curl http://localhost:8080/health` returns `{"status":"ok"}`. **Scope boundary:** do NOT change `rbp-server` source code (STW-101 is deploy + docs only, not new behavior); do NOT change the dashboard's `axum` router or its deploy runbook; do NOT change the trainer's `cargo test` invocations; do NOT change the existing README sections (the new `## HTTP API` section is *additive* — the existing `## Quick Start` + `## TUI Preview` + `## Testnet launch proof` + `## Public dashboard` + `## Workspace parallel test proof` sections are unchanged); do NOT touch the receipt chain, the v1/v2/v3 profile schema, the autotrain pipeline, the seat-aware work, the runbook, or the publish/index/remote chain. **Owner files:** `scripts/deploy-server.sh` (new — the operator-runnable deploy runbook, ~80 lines), `scripts/deploy-server.md` (new — the env-knob table + the operator-workflow walkthrough, ~80 lines), `README.md` (add the `## HTTP API` section with 24 routes + 24 curl examples, ~250 lines; the section is placed between `## Public dashboard` and `## Workspace parallel test proof` so the public-surface narrative flows: dashboard -> server -> tests), `crates/autotrain/tests/script_shape.rs` (extend the `runbook_doc_lists_every_env_knob` pin + add the `readme_has_http_api_section` pin + add the `deploy_server_script_parses_with_bash_n` sub-test, ~10 lines), `IMPLEMENTATION_PLAN.md` (this row), `steward/HAZARDS.md` (the `STW-082` + `STW-083` rows flip from "open" to "closed by STW-101 on <date>"). **Dependencies:** the existing `rbp-server` source code (the binary STW-101's deploy runbook builds + runs); the existing `scripts/deploy-dashboard-cloudflare.sh` (the shape-pin pattern STW-101's `deploy-server.sh` mirrors); the existing `README.md` (the section-placement model STW-101's `## HTTP API` follows); the existing `crates/autotrain/tests/script_shape.rs` (the env-knob + shape-pin pattern STW-101 extends). **Estimated scope:** M (one new shell script + one new doc + one new README section with 24 routes + 1 script_shape extension = ~410 lines of new artifacts). **Completion signal:** a fresh `bash scripts/deploy-server.sh` + a `curl http://localhost:8080/health` returns `{"status":"ok"}` on a local Postgres; a fresh `curl http://localhost:8080/api/index` returns the same `INDEX.json` shape the trainer's `INDEX.json` produces (the server is now self-evidently the public surface the testnet claim is about); the README's `## HTTP API` section is greppable + a stranger can `curl` any of the 24 routes with the documented shape. **`lens:` CEO (the testnet claim requires a public surface; the dashboard is half the surface, the server is the other half — STW-101 ships the deploy + docs the server side of the public surface needs; STW-082/083 from RE-PLAN-005 were the right *idea* but the wrong *timing* — the slice needed to land AFTER the STW-100 server tests, not before) + Eng (the deploy runbook mirrors the dashboard's; the README section is 24 route rows; the script_shape extension is 10 lines; the slice is bounded and verifiable) + Design (a stranger opens the README + sees `## Testnet launch proof` (the receipt) + `## Public dashboard` (the bench cards) + `## HTTP API` (the server) — the three sections together are the *complete* public surface the testnet claim describes; without `## HTTP API` the README implies a server the README does not document).**

- [ ] **[P1] `STW-102` `PLAN-CHECKBOX-HONESTY-AUDIT-POST-STW-098` (NEW in RE-PLAN-010; the plan-hygiene slice that closes the 4 P0 ghosts the post-7bfde14 audit revealed + extends the gate to catch them mechanically).** The post-7bfde14 audit reveals 4 P0 ghosts in the plan: `STW-091` (line 11658) + `STW-092` (line 11660) + `STW-093` (line 11662) + `STW-094` (line 11701) are `[ ] [P0]` in the plan but `[x]` on `main` (commit 7bfde14 shipped the integrated STW-091 lookup-cap + STW-088 property-test layer + the working-tree close on 2026-06-10 21:37:12Z). The plan-staleness-gate does not catch them today: the gate's P0 pass scans only `genesis/plans/000-ceo-testnet-roadmap.md`, not `IMPLEMENTATION_PLAN.md`; the gate's P1 pass scans `[ ] [P1]` rows in the plan, not `[ ] [P0]` rows. The 4 flips are XS (4 `- [ ] **` -> `- [x] **(RETIRED 2026-06-11 by RE-PLAN-010; see evidence row below)**` prefix-only replacements on the 4 ghost rows, preserving the original `[ ]` body as evidence a future planner can audit against). The gate extension is a new `STW-102-P0-PASS` (the gate's P1 pass sibling, scanning `[ ] [P0]` rows in `IMPLEMENTATION_PLAN.md` against `[x] STW-NNN` + `RESCOPED <date> by (RE-PLAN-NNN|STW-NNN)` markers — ~15 lines added to `scripts/plan-staleness-gate.sh` + 1 new sub-test in `crates/autotrain/tests/plan_staleness.rs::gate_catches_p0_ghosts`). After STW-102 lands, the active `[ ] [P0]` queue in the plan is 2 rows (`STW-098` + `STW-099`, the post-STW-094 work) + the 4 P0-ghost `[x]` retirement rows + the 9 carry-over P1s + the 4 new RE-PLAN-010 P1 rows. **Owner files:** `IMPLEMENTATION_PLAN.md` (4 prefix-only replacements on lines 11658 + 11660 + 11662 + 11701), `scripts/plan-staleness-gate.sh` (new `STW-102-P0-PASS` block, ~15 lines), `crates/autotrain/tests/plan_staleness.rs` (new `gate_catches_p0_ghosts` sub-test, ~10 lines), `steward/DRIFT.md` (the 4 P0 ghost rows in the `Blocked / Deferred` table flip from "DRIFT" to "RESOLVED by STW-102 on <date>"). **Scope boundary:** do NOT flip any non-ghost row (only the 4 P0 ghosts STW-102 names are flipped; the 9 carry-over P1s + the 4 new RE-PLAN-010 P1 rows are unchanged); do NOT change the existing P0 pass (the existing pass scans the roadmap; STW-102's new pass scans the plan); do NOT change the existing P1 pass (STW-102's new P0 pass is structurally parallel but scans `[ ] [P0]` not `[ ] [P1]`); do NOT change the operator-decision rows (STW-097 handles those; STW-102 is the P0-ghost slice); do NOT change the receipt chain, the dashboard, the server, the trainer, the receipt chain, the publish/index/remote chain, the seat-aware work, the autotrain pipeline, or any `trainer --*` CLI. **Acceptance criteria:** `bash scripts/plan-staleness-gate.sh` exits 0 with `plan staleness gate complete: checked=N ghosts=0` (the 4 P0 ghosts the post-7bfde14 audit revealed are now mechanically caught by the new P0 pass + the 4 manual flips clear them); `cargo test -p rbp-autotrain --test plan_staleness` is green with the new `gate_catches_p0_ghosts` sub-test; `cargo test --workspace -- --test-threads=4` stays green; `cargo check --workspace`, `cargo fmt --check` stay green; `rg -n "^- \[ \] \*\*\[P0\]STW-091|^- \[ \] \*\*\[P0\]STW-092|^- \[ \] \*\*\[P0\]STW-093|^- \[ \] \*\*\[P0\]STW-094" IMPLEMENTATION_PLAN.md` returns 0 matches (the 4 ghosts are `[x]`). **Dependencies:** the existing `scripts/plan-staleness-gate.sh` (the new P0 pass mirrors the existing P1 pass's regex + claim-map structure); the existing `crates/autotrain/tests/plan_staleness.rs` (the new sub-test mirrors the existing `gate_catches_p1_ghosts` sub-test's structure). **Estimated scope:** XS (4 prefix-only replacements + 15 lines of new gate code + 1 new sub-test = ~30 lines of new artifacts). **Completion signal:** `bash scripts/plan-staleness-gate.sh` exits 0 with no new ghosts; the next `auto steward --report-only` pass sees a clean P0 queue (2 rows: `STW-098` + `STW-099`); the post-STW-102 plan has no `[ ]` rows that duplicate shipped STW items. **`lens:` CEO (the planner's commitment is "the plan matches reality"; STW-102 is the slice that makes the commitment mechanically enforceable for the P0 surface the gate's existing P1 pass does not cover — the next RE-PLAN cannot re-introduce the 4 P0 ghosts the 7bfde14 commit + the post-STW-098 receipts reveal) + Eng (the 4 flips are prefix-only; the gate extension is structurally parallel to the existing P1 pass; the sub-test is 10 lines; the slice is bounded and verifiable) + Design (a planner opening the post-STW-102 plan sees a *clean* P0 queue: 2 active rows + 4 retired ghosts with `closed by STW-102` markers; the gate is mechanically catching the 4 ghost families the prior RE-PLANs caught by hand).**
